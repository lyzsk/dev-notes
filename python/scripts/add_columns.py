...
#     general     常规, 同 text
#     number      数字, 参数为小数位数(默认 2): 数量:number:2 -> 0.00; 数量:number:0 -> 整数
...
#   类型也接受中文: 文本 / 常规 / 数字 / 公式 / 日期 / 货币 / 单选 / 多选
...
def apply_column(ws, col, spec, max_row, header_style):
    """写入一个新列: 表头样式+文字, 类型格式/公式, 下拉框, 列宽自适应"""
...
def add_columns(xlsx_path, columns, sheets=None, output=None):
    ...
    for sname in targets:
        ...
        added = [apply_column(ws, src_col + 1 + i, spec, max_row, style)
...
def insert_columns(xlsx_path, anchor, columns, side, sheets=None, output=None):
    ...
    for sname in targets:
        ...
        for i, spec in enumerate(specs):
            pos = insert_at + i
            ws.insert_cols(pos)
            added.append(apply_column(ws, pos, spec, max_row, style))
...
def add_index_column(xlsx_path, name="序号", sheets=None, output=None):
