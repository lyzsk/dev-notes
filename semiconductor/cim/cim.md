...
## EAP vendor & system support form

### EQP INFO (for EE)

| 字段         | 含义          | Vendor需填/确认 | 说明                           |
| :----------- | :------------ | :-------------- | :----------------------------- |
| Vendor       | 厂商          | ✅              | TEL, AMAT, LAM, KLA等          |
| Agent        | 代理商        | ✅              | 若通过代理采购                 |
| Model        | 型号          | ✅              | 设备销售型号                   |
| Sub Model    | 子型号        | ✅              | Model Variant, 影响EAP模板选择 |
| Platform     | 平台          | ✅              | 设备所属平台系列               |
| Sub Platform | 子平台        | ✅              | 细分平台版本                   |
| Tool Type    | 设备类型      | ✅              | 如 Etcher, Scanner, CMP        |
| Inch         | Wafer尺寸     | ✅              | 300mm / 200mm                  |
| Technode     | 工艺节点      | ✅              | 28nm, 7nm等, 影响Spec要求      |
| Country      | 产地国家      | ✅              | 影响出口管制与报关             |
| LP Qty       | Load Port数量 | ✅              | 硬件配置确认                   |
| RF Reader    | RFID读写器    | ✅              | 是否有RF Reader及规格          |
| 新旧机台     | New/Old       | ✅              | 新机或搬迁旧机                 |
| OLD EQP ID   | 原设备编号    | ⚠️ (仅旧机)     | Copy项目或二手设备时必填       |
| Refer EQP ID | 参考设备      | ⚠️              | 指定Copy哪台现有设备的配置     |

```txt
├── Vendor / Model / Sub Model ──→ 决定设备原生能力基线
├── Tool Type / Technode       ──→ 定义工艺Spec要求
└── LP Qty + RF Reader         ──→ 【关键分支点】载具识别策略
      │
      ├── ✅ 标准设备 (Full E87 + RF Reader)
      │     ├── 自动读取 Carrier ID → MES Validation → Lock/Alarm
      │     └── EAP: S2F49 / Standard Carrier Report
      │
      ├── ⚠️ 半标设备 (有RF Reader但E87不完整)
      │     ├── RF Reader可读ID，但缺少Access/Validate序列
      │     ├── EAP需自定义RCMD补偿缺失的E87流程
      │     └── 需在Refer EQP ID中标注"部分兼容"
      │
      └── ❌ 非标/老旧设备 (无RF Reader / 私有协议)
            ├── 方案A: 加装外部RF Reader + PLC网关 → 模拟E87行为
            ├── 方案B: Barcode Scanner + 手动输入 + 二次确认防错
            ├── 方案C: 纯手动模式 → MES打印标签 + 人工扫码绑定
            └── ⚠️ 必须在Remark中标注: "Non-Std, 需Custom Adapter"
                  ↓
            [OLD EQP ID / Refer EQP ID] ← 非标设备必填！
            用于追溯历史适配方案、复用Custom Driver、避免重复踩坑
```

| 维度       | 标准设备                      | 非标设备                               |
| :--------- | :---------------------------- | :------------------------------------- |
| 载具识别   | RF Reader + E87 全自动        | 无RF / 私有协议 / 仅Barcode / 纯手动   |
| 参考基线   | Model/SubModel 即代表能力     | 必须依赖 `Refer EQ ID` 或 `OLD EQP ID` |
| 防错机制   | 硬件级 Interlock (放错即Lock) | 软件级/管理级防错 (二次确认/扫码/标签) |
| 信息完整性 | Vendor 提供完整规格书         | 字段常为空或填 "TBD"，需人工补全       |

##### RFID读写器(RF Reader), 是安装在EQP Load Port / 机台内部的 自动化组件, 核心作用是识别载具(FOUP/Cassette) 身份, 实现 "货" 与 "信息" 绑定, 防止人为输入错误导致的混料或报废事故
...
| IO mode                | I/O方式             | ✅              | Active / Passive                              |
| Device ID              | SECS Device ID      | ✅              | S1F13中的Device ID                            |
| Port                   | TCP端口号           | ✅              | HSMS端口, 通常5000或其他                      |
| GEM Version            | GEM标准版本         | ✅              | E30/E37/E40/E94等版本号                       |
| Software Version       | 主机软件版本        | ✅              | 影响Bug List与兼容性                          |
| Firmware Version       | 固件版本            | ✅              | 控制器固件版本                                |
| SECS Manual & Log Path | 文档路径            | ✅              | SECS/GEM手册存放位置                          |
| GEM Manual             | GEM通讯说明         | ✅              | 必须提供PDF/Word版                            |
...
| VID List               | 变量ID列表          | ✅              | Variable ID映射表                             |
| CEID List              | 采集事件列表        | ✅              | Collection Event定义                          |
| Alarm List             | 告警列表            | ✅              | Alarm Code与严重等级                          |
| Remote Command List    | 远程命令列表        | ✅              | RCMD定义                                      |
| Recipe Spec            | Recipe规范          | ✅              | PP结构说明                                    |
| PP Format              | Process Program格式 | ✅              | Binary/XML/Text                               |
| Recipe File Sample     | Recipe样例文件      | ✅              | 至少提供一个完整Sample                        |

```txt
[COM Type] TCP/IP / RS232 / 🔴 私有总线(Profibus/CC-Link/OPC-UA)
    ↓                              ↓
[Conn-Mode]                     [🔴 Non-Std Protocol]
HSMS-SS / SECS-I / EDA           私有TCP/Modbus/OPC/厂商SDK
    ↓                              ↓
[GEM Version]                   [🔴 Custom Adapter Layer]
E30/E37/E40/E94...               需开发协议转换中间件
    ↓                              ↓
[PICS] ✅ 完整                  [🔴 PICS缺失/不完整]
    ↓                              ↓
[SEDD] ✅ 标准XML/Excel         [🔴 无SEDD → 人工逆向工程]
    ↓                              ├─ 抓包分析(Sniffer)
[VID/CEID/RCMD List] ✅          ├─ 查阅纸质手册/源码
标准映射表                        └─ Vendor现场配合逐条验证
    ↓                              ↓
[Recipe Spec + PP Format]       [🔴 Recipe非标]
✅ Binary/XML + Sample           ├─ 私有格式 → 定制Parser
                                  ├─ 无Sample → Vendor驻厂生成
                                  └─ 无Checksum → EAP侧增加校验逻辑
    ↓                              ↓
[标准EAP Template]              [🔴 Custom EAP / Wrapper]
直接配置部署                      独立开发或基于Gateway转换后接入标准EAP
```

| 维度     | 标准设备                       | 非标设备                            |
| :------- | :----------------------------- | :---------------------------------- |
| 协议层   | HSMS-SS / SECS-I 标准协议      | 私有TCP / Modbus / OPC-UA / 厂商SDK |
| 合规文档 | PICS 完整，SEDD 可导入         | PICS 缺失/不全，无 SEDD，需逆向工程 |
| 数据映射 | VID/CEID/RCMD 清单齐全         | 清单缺失或与实际不符，需抓包验证    |
| Recipe   | XML/Binary + Sample + Checksum | 私有格式 / 无Sample / 无校验机制    |
| EAP策略  | 标准 Template 配置部署         | Custom Adapter / Gateway / Wrapper  |
...
协议实现一致性声明, 是Vendor对SEMI标准(E30/E5/E37等)的逐条合规性自测报告

以表格形式列出标准中的每一个条款(Clause), 标注"Mandatory/Optional/Not Supported", 并对所有 "Partial Support"或"Deviation"给出详细说明

EAP判断EQP能力边界, e.g. 是否支持S2F49增强型RCMD? 是否支持Block Transfer?

#### SEDD (SECS Equipment Data Dictionary)

SECS设备数据字典, 描述设备内部数据模型的元数据结构文件, 通常为XML或Excel格式

定义了所有Status Variable (SV)、Equipment Constant (EC)、Trace Variable (TV) 的层级关系、数据类型、单位、取值范围及关联的CEID

- EAP用来自动化生成EAP内部的变量映射配置, 避免人工录入数千个VID导致的错误
- FDC/EDA系统直接导入SEDD构建数据采集模型

#### VID List(Variable ID Mapping Table)

变量ID映射表, 设备内部变量名与SECS-II消息中数字ID的对照清单

包含 VID编号、变量名称、数据类型、读写属性(R/W/R/O)、关联EC/SV/TV分类、默认值、工程单位

- EAP通过S2F13/F14读取EC、S2F17/F18设置EC时，直接使用VID作为索引
- 状态监控、Alarm处理、Report生成均依赖VID进行数据解析

#### CEID List(Collection Event Definition)

采集事件定义表, 触发Host数据采集的事件触发器清单

CEID编号、事件名称、触发条件描述、关联的RPTID(Report ID)、每个RPT中包含的VID/DVID列表

- EAP通过S2F33/F34定义Data Link，将CEID与需要上报的数据绑定
- 当设备发生CEID对应事件时，自动发送S6F11/F12携带预定义数据

#### Remote Command List(RCMD Definition)

远程命令定义表, Host对设备进行反向控制的指令集

CMD ID、命令名称、参数字段(Name/Type/Required/Range)、执行前置条件、预期响应、超时时间

- EAP通过S2F41/F44下发控制指令(如Start、Stop、SelectRecipe、ClearAlarm、OpenDoor)
- 是实现MES Dispatch、Auto Scheduling的核心接口

#### Recipe Spec(PP Structure Description)

Process Program结构规范, 描述设备配方文件的逻辑组织架构

Recipe层级结构(Main/Sub/Step/Parameter)、参数命名规则、继承/覆盖机制、版本管理策略、Body大小限制

- RMS系统据此解析和管理Recipe内容
- EAP在执行Recipe Select/Download时，依据Spec构造正确的S7/S8消息序列

#### PP Format(Binary / XML / Text)

Process Program传输格式, Recipe Body在SECS-II消息中的编码方式

| 格式     | 优点                     | 缺点                                 | EAP处理复杂度         |
| :------- | :----------------------- | :----------------------------------- | :-------------------- |
| Binary   | 传输效率高，体积小       | 不可读，调试困难，强依赖Vendor SDK   | 🔴 高(需专用Parser)   |
| XML      | 可读性好，自描述，跨平台 | 体积大(约Binary的3-5倍)，传输慢      | 🟢 低(标准XML解析)    |
| Text/CSV | 简单直观                 | 无Schema约束，易出错，不支持复杂结构 | 🟡 中(需自定义解析器) |

### After Hookup Before EAP Release (for EE, PE, IT)

| 字段                       | 含义              | 填写人       | 说明                             |
| :------------------------- | :---------------- | :----------- | :------------------------------- |
| Seq                        | 序号              | PM           | 项目编号/行号                    |
| Base Line                  | Baseline          | PM           | 所属计划版本                     |
| Module                     | 制程模块          | EE           | Litho, ETCH, CMP等               |
| EQP ID                     | 新设备编号        | CIM/MES      | MES/EAP中的正式Tool ID           |
| Tool ID2                   | 第二编号          | CIM          | 备用ID/资产号/Vendor SN映射      |
| 实施方案-平台              | Solution Platform | Automation   | EAP解决方案平台                  |
| 实施方案-单元              | Unit              | Automation   | 具体实施单元                     |
| 实施方案-模块              | Module            | Automation   | EAP内部模块分类                  |
| 是否在实施方案             | 纳入项目标记      | PM           | Y/N                              |
| NewType-Rollout            | 新机种导入        | EE           | 是否首次导入该Model              |
| Experienced                | 已有经验          | EE           | 团队是否有类似设备集成经验       |
| EE Owner                   | 设备负责人        | EE           | Equipment Engineer               |
| PE Owner                   | 工艺负责人        | PE           | Process Engineer                 |
| IT Owner                   | 自动化负责人      | Automation   | Automation/CIM Engineer          |
| Vendor Owner               | 厂商窗口          | Vendor       | 厂商技术支持对接人               |
| Owner                      | 总负责人          | PM           | 项目Owner                        |
| 设备中文名                 | 中文名称          | EE           | 内部沟通用                       |
| Remark                     | 备注              | All          | 特殊说明/风险点                  |
| 装机 & 验机 Timeline-      |                   |              |                                  |
| T0 Start Date              | Hookup开始        | EE/Facility  | 水电气接入开始日                 |
| T0 Hookup Time             | Hookup耗时        | EE/Facility  | 天数                             |
| T1 Start Date              | 安装调试开始      | Vendor/EE    | SAT/FAT开始日                    |
| T1装机调试Time             | T1耗时            | Vendor/EE    | 安装天数                         |
| T2 Start Date              | Offline验机开始   | PE           | PE开始验机日期                   |
| T2 offline验机耗时         | T2耗时            | PE           | PE验机天数                       |
| T2 Release Date            | T2放行日期        | PE           | PE Sign-off日期                  |
| Request Pi Lot Date        | PI Lot申请日      | PE           | Process Integration Lot          |
| EQP Dry Run date           | Dry Run日期       | EE           | Dummy Wafer跑片测试              |
| EQP Test Recipe Setup Date | 测试Recipe建立    | PE           | 建立EAP专用Test Recipe           |
| EQP杀毒时间                | 杀毒完成日        | IT Security  | Windows设备安全合规检查          |
| EAP开发与集成              |                   |              |                                  |
| Need EAP                   | 是否需要EAP       | Automation   | Y/N                              |
| EAP Dev Start Date         | EAP开发开始       | Automation   | 代码/配置开发启动日              |
| EAP Dev Cost(Day)          | 开发天数          | Automation   | 预估/实际开发工时                |
| EAP Dev Finish Date        | 开发完成日        | Automation   | 开发结束进入测试                 |
| SECS Test Start Date       | SECS联机测试开始  | Automation   | 与机台实际联调日期               |
| SECS Test Cost(Day)        | SECS测试天数      | Automation   | 联调耗时                         |
| EAP Tuning Cost(Day)       | 调校/Bug Fix天数  | Automation   | 问题修复耗时                     |
| MES Integration Cost(Day)  | MES整合测试天数   | Automation   | Host端联调耗时                   |
| EAP Test Cost(Day)         | EAP验证/UAT天数   | Automation   | 用户验收测试耗时                 |
| EAP Release Date           | EAP正式上线日     | Automation   | Production Release日期           |
| GAP(EAP/T2 Release)        | T2到EAP上线间隔   | PM           | 衡量自动化就绪延迟的关键指标     |
| Non-EAP开发与集成          |                   |              |                                  |
| EDC                        | 是否EDC           | Automation   | Equipment Data Collection        |
| FDC                        | 是否FDC           | FDC Engineer | Fault Detection & Classification |
| RMS                        | 是否RMS           | Automation   | Recipe Management System         |
| APC                        | 是否APC           | APC Engineer | Advanced Process Control         |
| Run mode                   | 运行模式          | EE           | Online / Offline / Standby       |
| Need FDC                   | FDC需求确认       | FDC          | 详细FDC实施标记                  |
| EDA Model                  | Interface A模型   | FDC/EDA      | E164模型是否就绪                 |
| Metadata                   | 元数据            | FDC/EDA      | 变量描述与单位                   |
| Data Collection Plan       | 数据采集计划      | FDC/EDA      | DCP定义                          |
| Trace Variable List        | Trace变量清单     | FDC/EDA      | 需要高频采集的SVID/DVID          |
| Sampling Rate              | 采样率            | FDC/EDA      | 1Hz/10Hz/Event-based             |
| Trace Event                | Trace触发事件     | FDC/EDA      | 何时开始/停止Trace               |

```txt
[起点] EQP 到厂 & Hookup 完成
│
├── ✅ 标准设备 (Full GEM + E87 + SEDD齐全)
│    │
│    ├── T0 → T1 → T2 顺利推进
│    │    └─ T2 Release Date 可靠，GAP可控(目标 ≤ 0~3天)
│    │
│    ├── EAP开发路径：
│    │    ├─ 复用已有Template → Dev Cost低
│    │    ├─ SECS Test：标准PICS/SEDD支持，联调快
│    │    └─ MES Integration：协议一致，Host端无需适配
│    │
│    └── Advanced Apps (FDC/RMS/APC)：
│         ├─ EDA Model (E164) 可直接导入SEDD
│         ├─ Trace Variable List 由SEDD自动生成
│         └─ Data Collection Plan 基于标准DCP模板
│
└── ⚠️ 非标设备 (Non-Std / Legacy / Custom / Retrofit)
     │
     ├── 【关键触发条件】
     │    • NewType-Rollout = Y 且 Experienced = N
     │    • Remark 含 "Custom Adapter", "No PICS", "Manual Carrier"
     │    • Refer EQP ID 指向已知非标机台(如 EQP-OLD-203)
     │
     ├── Timeline 风险放大器：
     │    ├─ T1耗时 ↑：Vendor需现场调试私有协议 → 依赖Vendor人力
     │    ├─ T2 Release Delay：PE验机发现控制/数据异常 → 需EAP临时补救
     │    └─ GAP(EAP/T2) 显著拉长(常 > 14天)，PM需提前预警
     │
     ├── EAP开发路径(补偿机制)：
     │    ├─ 方案1：Custom Adapter Layer(独立进程/Service)
     │    │     • 输入：私有协议(Modbus/OPC-UA/厂商SDK)
     │    │     • 输出：模拟HSMS-SS + 标准SECS-II消息
     │    │     • 开发成本高 → EAP Dev Cost(Day) ×1.5~2.0
     │    │
     │    ├─ 方案2：Gateway模式(如 Cimetrix CIMConnect / KLA EDA Gateway)
     │    │     • Vendor提供Gateway配置文件
     │    │     • Automation仅需对接Gateway标准接口
     │    │     • 仍需验证Gateway行为一致性(常见Bug源)
     │    │
     │    └─ 方案3：人工干预兜底(仅限Low-risk设备)
     │          • EAP提供UI手动触发RCMD/Recipe Upload
     │          • MES增加二次确认弹窗 + Audit Log强制记录
     │
     ├── Advanced Apps 非标适配：
     │    ├─ FDC：无SEDD → 需FDC Engineer手工建模元数据(Metadata)
     │    ├─ RMS：无Recipe Spec → 依赖Vendor提供Sample逆向解析结构
     │    ├─ EDA Model：Interface A缺失 → 改用Polling或Event-Driven Custom API
     │    └─ Trace Variable List：靠抓包+工艺经验推断 → 高误报率风险
     │
     └── 关键责任人联动：
          • EE Owner：确认Run Mode是否支持Offline调试(非标设备常需Standby模式联调)
          • IT Owner：审批Custom Adapter的网络策略/安全豁免(如开放非标端口)
          • Vendor Owner：必须驻场支持T1~T2期间的协议调试
          • PM：在Remark中明确标注"Non-Std Risk: High"，并启动变更控制流程
```

| 维度          | 标准设备                      | 非标设备                               |
| :------------ | :---------------------------- | :------------------------------------- |
| Timeline      | T0→T1→T2→EAP Release 线性推进 | T1/T2 耗时不可控，GAP(EAP/T2) 常 >14天 |
| EAP Dev Cost  | 基线工时（如 10人日）         | ×1.5~2.0（含 Adapter 开发+测试+文档）  |
| SECS Test     | 自动化脚本覆盖 90%+           | 手动逐条验证，覆盖率常 <70%            |
| Advanced Apps | SEDD 自动导入，标准 DCP       | 手工建模元数据，靠抓包推断 Trace 变量  |
| 责任人联动    | 按标准流程分工                | EE/IT/Vendor 需深度介入，PM 每日跟踪   |

## EQP Move In Preparation

| 时间节点(Move In = T) | EAP工作                                              | FDC工作             | 输出物              |
| --------------------- | ---------------------------------------------------- | ------------------- | ------------------- |
| T-90天                | 收集设备资料（SECS/GEM、EDA、Recipe、Alarm、CEID等） | 确认EDA支持能力     | Interface Checklist |
| T-75天                | 厂商Kickoff、确认通讯方式                            | Metadata确认        | Meeting Minutes     |
| T-60天                | SECS/GEM开发                                         | EDA Model分析       | Interface Design    |
| T-45天                | Simulator联调                                        | Metadata导入        | Offline Test        |
| T-30天                | FAT（Factory Test）                                  | Trace配置           | FAT Report          |
| T-21天                | Recipe Mapping                                       | Sampling Plan确认   | Recipe Mapping      |
| T-14天                | MES联调                                              | Data Collection验证 | SAT准备             |
| T-7天                 | SAT                                                  | FDC SAT             | SAT Report          |
| Move In               | 上线支持                                             | 上线支持            | Release             |
| Move In+7天           | 稳定性观察                                           | Trace优化           | Close Report        |

### Phase 1

资料准备(Requirement & Interface Preparation)(T-90 ~ T-60)

目标: 完成Vendor资料收集、接口能力确认、实施风险评估，为EAP/FDC开发做好准

1. Equipment Information Collection 对应 [EQP INFO](#eqp-info-for-ee) 的 Vendor, Model, Platform, Tool Type, LP Qty, RF Reader, Refer EQP, OLD EQP, New/Old Tool, 判断设备属于标准机还是非标机，是否可以复用已有Template
2. Communication Capability Confirmation 对应 [Before T1](#before-t1-for-eap) 的 COM Type, Conn Mode, Device ID, Port, GEM Version, Software Version, Firmware Version, 判断接口实现方式
3. Interface Document Collection 对应 [Before T1](#before-t1-for-eap) 的 GEM Manual, Interface Spec, PICS, SEDD, VID, CEID, Alarm, RCMD, Recipe Spec, PP Format, Recipe Sample, 没有这些资料，EAP/FDC根本不能开始开发
...
| Need EAP              |   Y    |
| Need FDC              |   Y    |
| Support SECS/GEM      |  Yes   |
| Support EDA           |  Yes   |
| Support E87           |   No   |
| Support E90           |  Yes   |
| Need Custom Adapter   |   No   |
| Need Gateway          |   No   |
| Reuse Existing Driver |  Yes   |
...
| 无PICS      |  High  | Vendor补充     |
| 无SEDD      |  High  | FDC手工建模    |
| RS232       | Medium | Gateway        |
| 无RF Reader | Medium | Barcode方案    |
| 新Model     |  High  | 新开发Template |

### Phase 2

接口开发(Development & Configuration)(T-60 ~ T-30)

目标: 完成EAP/FDC系统开发、配置及内部验证，使设备具备进入Offline Test和FAT阶段的条件
...
接口验证(Offline Validation & FAT)(T-30 ~ T-14)
...
现场联调(Site Integration & SAT)(T-14 ~ Move In)
...
上线支持(Stabilization & Optimization)(Move In +7 Days)
