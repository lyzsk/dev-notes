...
- **MES 联动处置**: 通过接口对接 MES 支持 Hold Lot/Hold Tool/Hold Chamber/Hold Recipe, 并支持 Stop Tool/Stop Chamber 及客制化 OCAP;接口对接范围涵盖 MES/EAP/AMS;支持增加其他 Parameter 作为 OCAP 触发的附加判断条件.
...
- **数据查看**: 1. 可按照百分比去卡控, 例如在 0 W 等低 Target 的情况下设置下限防止 OOC/OOS 误报; 2. 直接筛选展示所有 OOC/OOS 的 Chart; 3. 在模型设置的界面上增加配置上次更改的用户信息, 目前做在版本控制界面可查询但不方便, Remark 与 Comment 功能 (已有); 4. Running Data 添加只导出列表 List 的选项, 做 Summary 的数据, 不导出全部数据.
...
- **统一权限认证**: 单点登录 SSO，基于域账号统一身份管理，实现 MES, SPC, RPT, FDC 等 CIM 子系统一键统一登录，一次认证、多系统免密访问.
...
- **签核协同**: 产生 OCAP 后与质量异常单签核业务接口整合, 将批次信息 (批次、段、站点、OCAP 原因等) 传递至外部签核系统, 指定步骤 (如判断重测是否 OOC/OOS) 支持传递至签核系统.
...
- **PM权限设置**: PM 查看权限数需多于可编辑权限数; 通过 Link 签核系统下发编辑权限.
...
- **AGV对MES的需求**: 提供与 AGVC 的通信能力, MES 为 Active (主动端).
- **任务下达**: 可向 AGVC 下达任务, 包含任务 Id, 物料号, 起点、终点、优先级.
- **储位分配**: 支持接收电子料架状态并对任务分配具体的储位.
- **料架储位状态显示**: 支持 MES 界面显示料架的储位的状态.
- **异常任务处理**: 具备异常任务处理机制 (MES 接收到 AGVC 上报的任务进度及状态后做出相应的处理).
- **任务反馈与履历**: 支持接收 AGVC 任务反馈, 并支持查询以及保存历史记录.
- **任务控制指令**: 可下达任务控制指令 (如 Cancel, Abort), 确保 AGVC 可以收到且正确分析指令.
- **任务目的地变更**: 在 AMR 自动搬送小车需要更改任务目的地的场景下, 支持先下 Abort 指令, 再重新创建任务.
- **料架载具信息接收**: 支持接收电子料架上报的载具信息 (如载具清单、ID、编号、类型、当前位置编号、当前区域编号).
- **批量任务下达**: 支持针对 Batch 设备的货物运输下达多个任务指令.

### 58. MCS 模块

MES 与 MCS (天车搬送控制) 系统的模块需求.

- **任务重发幂等**: 支持因网络超时、界面重试或人工操作等异常情况再次发送同一笔搬运任务需求, 此时需沿用原任务标识或明确提供原任务关联标识, 不得生成一笔看似全新的搬运.
- **任务冲突处理**: 支持 MES 发现针对目标下达的新任务与旧任务的冲突; 当两个任务部分信息 (如 Carrier、来源、目的地等) 不一致时, 不能直接改已预约 Job 里面的值, 需根据流程 Reroute、或 Cancel 后新建任务指令、或明确提示业务冲突.
- **任务状态区分**: 针对 MCS 返回的各类信息 (如“通信已收到”、“任务已受理”、“执行中”、“完成”、“失败”等), MES 需分别记录并展示其不同的业务含义, 不能据此判定任务已完成或 Carrier 已到达.
- **请求受理结果处理**: MES 需取得 MCS 对搬运请求的接受或拒绝结果并对回复进行处理; 如果 MCS 拒绝执行命令, MES 需按流程决定人工处理、调整后重送或停止派工.
- **搬送事件处理**: MES 可接收 MCS 上报的信息 (Carrier、任务标识及事件标识, 如“搬送信息”、“位置变化”、“到位”、“失败”、“取消”等), 并依据此类信息处理事件, 保证业务信息状态可维护追溯.
- **重复乱序事件识别**: MES 收到特殊情况 (断线恢复、系统重启或补发) 导致的重复、迟到或乱序事件时, 能识别已处理事件或旧版本状态, 不得因重复事件再次下达搬运, 也不得以旧状态覆盖新状态.
- **搬运任务取消**: MES 能取消尚未完成的搬运任务, 取消请求需指向原任务标识, 且 MES 需等待接收 MCS 的回复信息 (如“已接受取消”、“无法取消”、“已完成”、“已终止”等结果).
- **目的地变更**: 当 Carrier 目的地改变后, MES 需提供原任务标识、新目的地及变更版本或顺序信息, 将同一 Carrier 改送至新目的地, 且需等待天车 MCS 对更改目的地结果的明确回应.
- **任务优先级调整**: 当 MES 需调整未完成任务的优先级时, 优先级调整必须指向已有任务, 不得把旧任务 Cancel 后新建任务替代; 同时 MES 需接收变更已接受、未生效或不支持的结果.
- **多载具批量任务**: MES 可一次下达多个 Carrier 的搬运任务, 且必须选择该批任务是“整批预检均通过才执行”或“允许逐笔接受 / 拒绝”, 每个 Carrier 都需有独立的结果.
- **位置上报与对账**: MES 支持实时接收 MCS 上报的 Carrier 位置变化, 必要时可按 Carrier 查询位置或参与项目定义的库存 / 位置对账流程.
- **搬送属性管理**: MES 必须明确维护各类业务属性 (Carrier Type、批次属性或其他搬送判断所需信息) 的属性来源与更新时机, 并有明确的属性缺失处理规则, 属性缺失时不得下达可能错误的搬送任务指令.
- **跨MCS中转搬送**: 同一 Fab 存在两套 MCS、搬运起点与终点分属不同 MCS 时, 需经中转 STK 或交接站点; MES 或项目约定的协调组件需维护设备归属与交接站点规则, 跨区任务不得直接要求两套 MCS 同时执行同一段物料搬送; 中转前段完成、Carrier 到达交接位置并满足续派条件后才可触发后段.
- **跨区派工异常管控**: 支持在收到交接站点、站点所属设备或归属信息异常时, MES 停止受影响的跨区派工并保留待处理任务.
- **断线重连与对账**: 面对特殊情况 (如 MES 与 MCS 连接中断、网络抖动、MES 重启或 MCS 服务切换), MES 需自动重连至项目约定的 MCS 逻辑服务地址; 恢复后使用原任务标识查询结果、接收补发事件或进行对账.
- **未知结果处理**: MES 必须有对未知情况 (如已发送请求但未收到业务回复、已收到事件但无法确认是否完成处理等) 的处理机制: 优先使用原任务标识查询或等待补发, 不得直接以新任务标识重复下单.
- **自研中间件规格**: MES 使用自研或非通用通讯中间件与 MCS 连接时, MES 厂商需提供中间件技术规格、连接与订阅方式、报文样例、错误码、超时重试、断线重连、版本规则, 以及可供联调的环境、模拟工具或技术支持窗口; MCS 可配合适配; 中间件能力说明、缺陷定位及中间件侧修正由 MES 厂商负责.
- **通信与ACK语义**: MES 与 MCS 采用请求 / 回复、事件上报或 ACK 确认方式通信; MES 厂商需明确定义 ACK 仅代表“通信收到”、还是“进入处理”或“业务接受”, ACK 与业务完成不得混用.
- **逻辑地址接入**: 在网络、服务器或接口实例切换中, MES 不得固定连接单一 MCS 物理服务器, 需使用双方 (MES & MCS) 约定的逻辑地址、虚拟地址、服务名称或等效接入方式.
- **版本发布协同**: MES 厂商发布版本更新 (如接口字段、枚举、通讯中间件等) 前, 需向天车厂商提供版本号、差异说明、受影响接口、样例报文、兼容策略、回退方式及回归测试计划.
- **联调测试支持**: 项目进入接口联调、FAT, SAT 或生产切换时, MES 厂商需提供接口技术窗口、问题响应机制、测试账号 / 环境、测试数据准备及上线支持安排.
- **容量性能承诺**: 在特定生产情景下 (如正常生产、满载生产、短时突发或断线后事件补发等), MES 厂商需提供其可承受的请求量、事件量、同时未完成任务数、事件积压与恢复补发能力, 并参与项目容量测试.
- **异常日志与告警**: 发生异常状况时 (如接口错误、积压、重试过多、连接异常或业务拒绝率异常等), MES 需提供必要日志、追踪标识、连接状态、积压量、错误码及告警信息, 供 MES 和天车联合定位; 敏感信息需按项目要求脱敏.

### 59. 其他业务协同

- **与泛微门户协同**: 需包含门户管理、协同管理等模块协同.
- **与用友 ERP 协同**: 需包含财务管理 (工单 / 物料 / 成本) 等模块协同.
- **与 WMS 协同**: 需包含物料出入库、库存管理等模块协同.
- **与 PLM 协同**: 需包含产品数据、工艺数据、BOM 管理等模块协同.
