...
- **多对象叠图**: 不同 Chamber/Tool 可放在同一张 FDC Chart 查看;支持分设备, 腔室叠加追踪数据 (Trace Data) 与汇总数据 (Summary Data);支持对 Raw Trace 数据一键叠图 (Lot to Lot, Wafer to Wafer, Step to Step), 查看不同 Lot/Slot/Substrate/Step 之间的数据差异并找出异常对象.
...
- **Rawdata 存储**: 支持 Rawdata 分表存储, 文件存储;支持数据存储分级 (如最近 2 天, 一个月内, 六个月内, 一年内) 与高效的数据清理及恢复机制, 保障系统性能不随时间推移而下降.
