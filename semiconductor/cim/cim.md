...
| 工程与质量管理   | YMS, DMS, RPT           | YMS 组   |
| 实验室管理       | LIMS, FMS, QMS          | Others组 |

## 一套体系、五个业务域、六大要素

```txt
“一套体系”：即打造贯通“任务—工艺—生产—设备—物流—质量—实验—数据”的数字化CIM体系，实现科研中试全过程数字化、设备自动化、生产透明化、过程精细化和数据资产化的目标。对外通过统一门户与统一权限体系提供服务，用户单点进入，后端由多个专业系统分工承载。
“五个业务域”：即围绕“设备自动化与控制”、“生产执行与追踪”、“智能调度与物流”、“工程与质量管理”、“实验室管理”五大业务域持续建设和完善，以支撑“架构—设计—材料—工艺—器件”全流程研发和中试验证能力。五个业务域共用一套主数据、一套接口标准、一套权限体系与统一门户，做到数据同源、流程互通、视图统一，能力划分用于界定各系统的职责边界，避免同类功能重复建设。
“六大要素”：以“人、机、料、法、环、测”——“人员、设备、物料、工艺方法、环境、量测”六个生产要素为统一管控对象，五个业务域统一引用，以批次为主线，根据研发任务与中试任务分态留痕，实现六大要素数据的关联存储与追溯。
一套体系回答“建成什么”，五个业务域回答“谁来做”，六大要素回答“管什么”，三者协同最终服务于科研中试全过程数字化、设备自动化、生产透明化、过程精细化和数据资产化目标的实现，为平台长期演进预留扩展空间。
```
...
- **开放式架构**: 面向服务或消息驱动的开放式架构, 可安全便捷地与其他业务接口整合.
- **64位支持**: 支持 64 位系统.
...
1. 标准SECS设备：5W/type、0.5W/rollout；
...
```text
机台有以太网口 + 支持 HSMS ──→ 不需要 MOXA，直接网线接 Switch
机台只有 RS232 口 ──────────→ 需要 MOXA（或同类串口服务器）
机台有以太网口但只跑 SECS-I ─→ 极少见，需与 Vendor 确认
```

| 指标                    | 说明                                                    | 影响                                |
| :---------------------- | :------------------------------------------------------ | :---------------------------------- |
| 通讯物理接口类型        | RS232 / RS422 / RJ45 / 光纤                             | RS232/422 → 需要 MOXA               |
| 是否支持 HSMS           | 看 GEM Manual 或问 Vendor                               | 不支持 → 只能走串口                 |
| 串口数量                | 部分机台有 2 个串口（一个 SECS，一个 Terminal/Service） | 决定 MOXA 端口数（1口/2口/4口）     |
| 波特率                  | 9600 / 19200 / 38400 / 57600 / 115200                   | MOXA 必须支持对应波特率             |
| 流控方式                | Hardware (RTS/CTS) / Software (XON/XOFF) / None         | 配置 MOXA 时必须匹配                |
| 能否固件升级支持 HSMS   | 问 Vendor 是否有升级方案                                | 能升级 → 优先升级，避免 MOXA 中间层 |
| 是否有 FDC 高频采集需求 | Trace Data 采样率 > 1Hz                                 | RS232 带宽可能不够，必须评估        |
| 安装位置与机柜空间      | MOXA 需要 DIN 导轨或机架安装                            | 影响选型（导轨式 vs 机架式）        |
| 供电要求                | 单路 / 冗余双路                                         | Fab 通常要求冗余供电                |
...
```
SECS-I / HSMS(E37)   传输层: RS232 串口 或 TCP/IP 网口
SECS-II (E5)         消息层: SnFm 报文格式
GEM (E30)            状态模型: 通讯/控制状态机, 定义 Host 能看到什么
GEM300 (E39/E40/E87/E94)  对象服务: 载具/工艺任务/控制任务 (12寸全自动必需)
```

### SnFm 命名规则

- **S**tream = 功能类(如 S2=数据/配置, S6=数据采集, S7=配方), **F**unction = 类内序号
- **奇数 F = 主消息**(发起方→接收方), **偶数 F = 应答**. 例: S2F41 Host 发远程指令, 机台回 S2F42
...
| 项            | 标准 | 作用                                                 |
| :------------ | :--- | :--------------------------------------------------- |
| HSMS 连接参数 | E37  | IP/端口/设备ID/超时, 没它连不上机台                  |
| GEM 状态模型  | E30  | COMM/EQUIPMENT/HOST 状态机, Online Local/Remote 切换 |

**② EC 设备常数(配置下发)**

| 消息          | 作用                                                       |
| :------------ | :--------------------------------------------------------- |
| S2F13 / S2F15 | 读/写 ECID(Equipment Constant), EAP 下发机台配置参数的通道 |

**③ 数据定义(EAP 取数基础)**

| 项    | 作用                                                                                |
| :---- | :---------------------------------------------------------------------------------- |
| SVID  | 状态变量表, S1F3 可随时读取(温度/压力/状态)                                         |
| CEID  | 事件 ID 表, 机台状态变化(Start/Complete/Load…)的编码                                |
| ALID  | 报警 ID 表                                                                          |
| RPTID | 报告 ID + 事件-报告关联(S2F33 定义/S2F35 关联/S2F37 启停), Data Collection 配置核心 |

**④ 控制指令(Host → EQP)**

| 消息  | 作用                                                    |
| :---- | :------------------------------------------------------ |
| S2F41 | RCMD 远程命令(START/STOP/ABORT/PP-SELECT), EAP 控机核心 |
| S7F19 | 查询机台当前配方/PPID 列表                              |
| S7F3  | Host 向机台下发配方                                     |
...
| 消息  | 作用                                                              |
| :---- | :---------------------------------------------------------------- |
| S6F11 | 事件报告, 携带 CEID + 关联 RPTID 的变量数据(EAP 监听机台的主通道) |
| S5F1  | 报警上报, 携带 ALID                                               |
...
| 消息   | 标准      | 作用                                              |
| :----- | :-------- | :------------------------------------------------ |
| S3F17  | E87 (CMS) | Carrier Action: Host 远程 Load/Unload/Cancel 载具 |
| S14F9  | E94 (CJ)  | Create Object: 创建 Control Job                   |
| S16F15 | E40 (PJ)  | Process Job 管理(Start/Create/Delete 工艺任务)    |

> 方向记忆: **④是 Host 发号施令, ⑤是机台主动汇报**. ①②③是地基(连得上/配得了/取到数), ⑥是全自动测机的门槛.

### SECS 手册完备度五档(评估厂商手册够不够 EAP 开发用)

| 等级          | 判据                                               |
| :------------ | :------------------------------------------------- |
| 完整 GEM300   | S3F17+S14F9+S16F15 齐备, 且有 EC、SVID/CEID、S2F41 |
| GEM300 较完整 | 三消息 2/3, 或三消息齐但缺 S2F41                   |
| 基础 GEM      | 有 SVID/CEID 数据定义, 无 GEM300 对象控制消息      |
| 简单 SECS     | 只有零星 HSMS/EC 信息, 不足以支撑开发              |
| 非 SECS 资料  | RS-485/编程/UI 类资料, 需向厂商另索 SECS 手册      |

> 注意: 手册里出现 "GEM300" 字样只是**声明**, 不等于三条消息真有定义, 必须逐一核实 S3F17/S14F9/S16F15.

### 检查项优先级 (P0/P1/P2) 与行动

| 检查项          | 优先级 | 手册必须包含               | 可测机验证       | 缺失后果               |
| :-------------- | :----- | :------------------------- | :--------------- | :--------------------- |
| HSMS 通讯       | P0     | ✅ IP/端口/超时参数        | ❌               | 无法建链, EAP 无法工作 |
| EC 设备常数     | P0     | ✅ ECID 列表及定义         | ❌               | 无法参数化配置设备     |
| CEID 事件定义   | P0     | ✅ CEID 列表及触发条件     | ❌               | 无法采集设备事件数据   |
| SVID 变量定义   | P0     | ✅ SVID 列表及数据类型     | ❌               | 无法获取工艺参数值     |
| RPTID 报告定义  | P0     | ✅ RPTID 与 CEID/SVID 映射 | ❌               | 无法配置数据上报       |
| ALID 报警定义   | P0     | ✅ ALID 列表及类型         | ❌               | 无法识别和处理报警     |
| E30 状态模型    | P1     | ✅ 声明支持                | ✅ 状态转换验证  | 无法正确监控设备状态   |
| S2F41 远程指令  | P1     | ✅ 声明支持+指令列表       | ✅ 具体指令验证  | 无法远程控制设备       |
| S6F11/S5F1 上报 | P1     | ✅ 声明支持                | ✅ 触发条件验证  | 无法接收事件/报警      |
| S7F3/S7F19 配方 | P1     | ✅ 声明支持                | ✅ 格式/流程验证 | 无法管理配方           |
| GEM300 三消息   | P2     | 按需                       | ✅ 按需验证      | 视项目需求而定         |

> 行动逻辑: **P0 是文档问题**(手册必须写清, 缺了找厂商补, 测机测不出来);
> **P1 是验证问题**(手册声明即可, 靠测机实测确认); **P2 是需求问题**(按项目是否全自动/12寸决定要不要).
...
- **配置化管理**:
    - 机台事件动态配置, 系统信息配置管理, 支持通过独立的配置文件进行管理.
    - 设备初始化以及工艺前校验模块的配置管理.
...
- **量测数据处理**: <mark>支持量测数据具体位置灵活配置, 支持量测数据过滤与数据转换.</mark>
- **数据采集与上报**:
    - 支持 SECS/GEM, TCP/IP, <mark>File, Database, FTP, PLC 等方式采集数据</mark>.
    - 支持设备事件, 报警, 状态, 常量, 工艺参数等数据采集.
    - <mark>支持数据格式转换与采集开关配置.</mark>
    - <mark>支持向 FDC, AMS, MES 等系统上报</mark>.
...
- **专项设备作业支持**:
    ...
    - <mark>支持 Bonding 和 Debonding 的 Job 信息请求、Job In、Job Out.</mark>
    - <mark>支持 MOCVD 设备的 Job 信息请求、Job In、Job Out.</mark>
    - <mark>支持 NPW 相关操作请求, Dummy In、Dummy Out、Monitor Start、Monitor End、Clean Start 和 Clean End 等功能.</mark>
- **设备状态同步**:
...
- **Recipe 解析上传**: <mark>支持对设备 Recipe Parameter, Recipe Body 的解析并上传至 RMS 系统.</mark>
...
- **校验配置**: <mark>支持 By LotType/StepId 进行判断是否需要进行 RMS 校验, 可通过配置文件配置.</mark>

### 3. 其他业务协同

- **与泛微门户协同**: <mark>需包含门户管理等模块协同.</mark>
- **与帆软报表协同**: <mark>需包含报表管理等模块协同 (预留, 优先经 MES 汇聚).</mark>
- **协同范围**: <mark>支持与 MES, RMS, FDC, APC, AMS, PMS, AMHS, RCM 等CIM其他业务接口整合.</mark>
- **接口方式**: <mark>支持 REST/Web API, TCP/IP, DB/File 等接口方式, 具体以客户 CIM 接口规范为准.</mark>
...
- **设备通讯异常智能诊断**: <mark>面向EAP与机台通讯中断、报文超时、SECS 指令无应答等异常，由 AI 关联分析事件前后的 SECSLog、StatusLog 与 ErrorLog 片段，给出可能原因的判断次序与排查路径；同类异常按设备类型归集，沉淀为可检索的诊断条目，供后续问题直接引用.</mark>
- **跑货异常处置辅助**: <mark>跑货中断或触发 Hold 时，AI 比对当前 Lot、设备与 Recipe 状态同最近一次正常生产的差异，输出处置建议与恢复步骤；值班人员执行后的处置结果与耗时回写，作为后续同类判断的样本.</mark>
- **设备对接文档智能解析**: <mark>新设备导入阶段，AI 读取厂商提供的 SECS/GEM 手册与设备规格文件，提取 SV、EC、CEID、ALID、RPTID 等定义及状态机行为描述，输出设备驱动配置草表与待确认事项清单，压缩人工整理的时间.</mark>
- **日志智能问答**: <mark>CIM运维人员以自然语言提出查询意图，如某设备在某时段内某类异常的发生次数与分布，由 AI 转换为检索条件在内网日志库中执行，并将命中结果整理为结论与依据清单；原始日志不出内网，仅统计结论与脱敏片段用于生成表述.</mark>
- **建议复核与知识沉淀**: <mark>AI输出的通讯异常诊断结论、跑货异常处置建议、手册解析草表，由 EAP 工程师在处置完成后回填采纳情况；未采纳的标注原因，区分结论不成立、与现行作业规范冲突、信息不足三种情形. 记录按设备 TYPE、厂商与异常类别归集，例如 S7F19 获取 PPID List 失败、S1F3 状态查询超时、Load Port E84 未完成、断线重连后的恢复处置，沉淀为可复用的诊断条目与处置步骤. 随样本积累，按各类情形的采纳率与误判分布反向修正提示内容与触发条件，新 TYPE 导入或同类异常再现时可直接调用既有条目.</mark>
- **vFAB 北向接口预留**: <mark> 支持事件、状态查询、受控动作、回写模型等接口预留 (包括但不限于设备/腔体状态切换、通讯断连与恢复、异常干预触发等事件发布, 设备实时状态、载具与 Load Port 状态、当前跑货 Job 与 Recipe（PPID）等状态查询, 跑货方案确认/驳回、异常干预指令受控下发等).</mark>
...
> 前置确认: Hookup 完成 / IP 配置 / Device ID / 通讯确认 → SAT → EAP 与上位业务接口整合联调 → FDC 联调.
...
- **多语言支持**: 默认支持用户选择语言 (中文 / 英文);支持系统亮 / 暗色模式切换.
...
- **FAB 管理**: 支持用户输入 / 导入 FAB 信息, 支持多个 FAB 同时使用且互不影响, 支持多产线同时使用.
...
- **同型同步**: 支持同类型设备的 SVID/DVID 的相互同步;支持设备或子设备之间的数据配置 (Raw Trace 参数, Event, Alarm, Summary 参数及虚拟参数) 快速批量拷贝.
...
- **虚拟参数**: 支持添加虚拟参数, 虚拟参数可通过图形化的组合, 支持常用的算数表达式, 并支持虚拟参数的模拟;支持建立虚拟追踪参数 (Virtual SVID), 虚拟事件 (Virtual Event), 虚拟事件参数 (Virtual DVID), 虚拟汇总参数 (Virtual Trace Summary);支持可视化工作流设计 (Graphical Workflow Designer) 创建虚拟数据的逻辑, 并支持历史数据的校验 (Workflow Validation);虚拟参数可作为实际数据使用, 包含实时监控, 建模与批量导出.
...
- **非标接入**: <mark> 支持从 File, DB, FTP, TCP/IP 中收集 Extra Sensor 数据.</mark>
...
- **PT 分流**: 能够提供 PT, 支持 EAP 的数据与 FDC 的数据进行分流;能对设备发送的 Message 进行分流过滤.
...
- **SPC Alarm Rule**: 支持 SPC Alarm Rules 及自定义 Alarm Rule; 支持基于算法, 参数设定 Alarm Rule 模板; 单个模型支持设置多个 Alarm Rule;支持连续 Y 个点中有 X 个点发生异常 (X<Y) 的监控模式.
...
- **分级报警**: 支持根据告警规则, 设定 Warning, Critical 以及 Outlier 的报警方式;可定义不同的报警 Level, 并根据不同的 Level 定义不同的通知方式.

#### 5.3 数据质量报警管理

支持数据采集质量的报警配置.

- **质量报警**: 支持根据设备类型设置数据采集的数据质量报警, 在没有达到采集的点数时进行报警的推送;支持自动计算追踪数据 (Raw Trace) 的数据质量并进行监控.
...
- **Context Group**: 支持 Context Group 的概念, 即类似的 Context 可以在一个 UVA 模型下进行分组, 并可以设置 Context 的优先级;支持 SPC 自动建模, 按 Context 组合 (如产品, 生产配方) 自动建立 Sub 监控模型并进行监控, Sub 模型自动生成并继承 main 模型的设置内容.
...
- **过滤条件**: 支持 SV, DV, 用户自定义过滤 Limit 以及用户自定义条件来进行数据过滤;支持依照不同类型的 Context 组合筛选 (Include) 或排除 (Exclude) 监控数据, 筛选条件支持 \* 模糊匹配.
...
- **历史模拟**: 支持历史数据模拟.
- **跨片统计**: 支持连续多片 Wafer 之间的模型统计值计算 (如连续两片之间 Moving Range), 并在 Lot ID 切换时重置; 统计算法包括 Mean, Max, Min, Sum, Slope, EWMA;支持对 Chart 数据进行二次汇总统计计算, 提供 10+ 种计算方法 (STDEV/MA/EWMA/Range 等).
- **步骤时间累加**: 支持对工艺中某个步骤的时间进行累加, 超过规格后报警并可复原;支持对 Inline 设备的 Process time, Process Step Time, Unit 之间的 Transfer time 进行计算并实施监控;支持监控设备之间 Wafer transfer 的运行时间以及设备部件的运行时间 (如 Transfer time).
...
- **规格种类**: 支持 N-Σ (目标值 ± N-Σ), Δ (目标值 ± Δ), 删除百分比 (目标值 ± 删除 × N%), 移动平均目标值 (用户定义 N 值);规格值计算支持百分比 (Percentage), 标准差 (Standard Deviation) 和固定值 (Constant) 等方式.
...
- **规格复用**: 模型规格支持复制粘贴. 需支持一键计算: Recipe 2 与 Recipe 1 的 Spec 一致时, Recipe 2 可一键沿用 Recipe 1 的 Spec 进行计算.

#### 6.7 UVA 模型管理

支持模型的集中管控与版本追溯.

- **集中管理**: 支持对模型的控制窗口进行统一管理, 可以进行集中修改, 并支持对模型进行快速复制, 可复制至其他设备, 子设备 (Module), 生产配方 (Recipe);支持模型分组 (Model Group) 管理.
- **履历与版本**: 支持保存修改历史, 能够对模型的修改履历进行查看, 并提供模型不同版本的对比功能;支持模型配置回滚 (Rollback).
...
- **AMS 报警协同**: 通过接口对接 AMS 系统支持 Email/SMS/Phone/Wechat 多种报警方式.
- **MES 联动处置**:
    - 通过接口对接 MES 支持 Hold Lot/Hold Tool/Hold Chamber/Hold Recipe, 并支持 Stop Tool/Stop Chamber 及<mark>客制化 OCAP</mark>;接口对接范围涵盖 MES/EAP/AMS;<mark>支持增加其他 Parameter 作为 OCAP 触发的附加判断条件.</mark>
...
- **Lot/Wafer 履历**: 支持以 Lot/Wafer 为单位查看设备加工履历, 查看加工过程中参数的趋势变化, 并结合模型数据进行分析;支持按照区域, 设备组, 设备, 子设备, Lot, 时间范围等条件查询生产历史, 并选中多个 Lot/Wafer 数据进行 Raw Trace 分析;支持按照设备, 腔室或 Lot ID 查询一个或多个 Lot 生产经过的机台 / 腔室路径, 并 Drilldown 到具体生产机台, 腔室的 Raw Trace 数据.
...
- **多对象叠图**: 不同 Chamber/Tool 可放在同一张 FDC Chart 查看;支持分设备, 腔室叠加追踪数据 (Trace Data) 与汇总数据 (Summary Data);支持对 Raw Trace 数据一键叠图 (Lot to Lot, Wafer to Wafer, Step to Step), 查看不同 Lot/Slot/Substrate/Step 之间的数据差异并找出异常对象. 支持关联 Chamber, 同一设备不同工艺的 Chamber 可一起查看.

#### 7.3 实时数据

支持跟踪参数的实时监控.

- **实时监控图表**: 支持在趋势图表中显示跟踪参数的实时值, 并提供牛眼图表和计量图;支持查看跟踪参数的实时值是否偏移控制限制或规格限制, 或是否保持在控制限制内;支持 by EQP/Chamber/Parameter 的实时 Trace 数据趋势图, 可对比多 Chamber, 多 Parameter;查看实时数据时, 支持召回当前 Lot/Substrate 的历史生产数据 (最多前 21 天) 并显示为 Trend Chart;支持预设实时数据显示模板 (定义关注的参数, 支持参数值作为 X/Y 坐标), 支持多 Y 轴显示实时数据.
...
- **Stats 趋势**: 支持根据时间查看 FD 模型中所有 Stats 的 Chart 图形的变化趋势, 并可以确认计算每个 Stats 的 Raw Data;支持按照区域, 设备组, 设备, 子设备, 汇总参数, 时间范围等条件查询至少六个月的汇总数据 (Summary Data) 并进行趋势分析;支持以子设备 (Module), 参数 (Parameter), 生产配方 (Recipe) 或生产步骤 (Recipe Step) 进行筛选与绘图.
...
- **分组统计**: 支持展示根据设备、配方以及统计值分组的 UVA 结果;支持统计不同机台每天报警次数 Top N 的模型;支持统计查询时间范围内报警次数 Top 5 的机台.
...
- **Cp 统计**: 支持为每一个设备或者 Model 计算 CP, CA, and CAP, 并且仅反馈某个设备的 CP 小于阈值的情况;支持计算 Ppk 并提供日报.

#### 7.10 UVA 配置分析

支持 UVA 设置情况的统计分析.

- **设置详情**: 支持按照机台腔查看 UVA 设置详情列表; 支持查看 UVA 设置覆盖率统计图表;覆盖率统计支持按照区域, 设备组, 设备, 子设备, 时间范围等条件查询, 包含各个等级的参数个数, 已建模的参数数量, FDC 模型数量, 激活数量, Interlock 启用数量.

#### 7.11 图表联动与导出

支持图表间的联动查询与数据导出.

- **图表联动**: 查看 UChart 时支持点击 Link 到 TChart; 查看警报记录列表时点击单条可直接在右侧弹出 UChart 绘图;SPC Chart 上的数据点支持直接 Link 到 Trace Data.
- **数据导出**: 所有列表支持数据导出. 支持 By 产品导出数据列表 (xlsx 格式), 仅导出列表不要具体数据.
- **数据查看**: <mark>1. 可按照百分比去卡控, 例如在 0 W 等低 Target 的情况下设置下限防止 OOC/OOS 误报; 2. 直接筛选展示所有 OOC/OOS 的 Chart; 3. 在模型设置的界面上增加配置上次更改的用户信息, 目前做在版本控制界面可查询但不方便, Remark 与 Comment 功能 (已有); 4. Running Data 添加只导出列表 List 的选项, 做 Summary 的数据, 不导出全部数据.</mark>
...
支持拖拽式图形化工作流平台, 实现低代码业务定制与第三方接口对接.

- **工作流平台**: 具备图形化工作流平台, 支持基于当前流程创建新流程.
- **第三方协同**: <mark> 支持利用图形化接口定制与第三方接口能力.</mark>
- **外部访问能力**: 具备数据库访问, 访问外部 WebService 等能力.
- **业务接口实现**: <mark> 支持利用工作流实现 Query Lot Process Context 接口业务.</mark>
...
- **Service 监控**: 提供 Service 监控管理工具;支持在 UI 界面远程启停后台应用, 并具有完善的操作记录.

#### 11.3 高可用与高并发

支持高并发数据处理, 保障系统高可用与平台可监控性.

- **高可用机制**: 支持负载均衡与故障自动切换; 支持跨多环境, 多服务的统一配置管理, 提高系统一致性.
- **平台监控**: 提供系统平台监控工具, 对 OS 和运行程序全方位监控和预警;支持分析后台应用的中长期性能指标, 包含应用各模块处理的消息量, 线程使用数量, 数据库连接数量, 数据平均处理时长等, 所有数据支持 Trend Chart 显示;支持对服务器重要性能指标, 后台应用重要性能指标设定监控规格, 超出监控规格可以发送报警;支持查询性能指标报警历史记录.

#### 11.4 数据库与存储

支持多数据库选型与开放的数据架构.

- **数据库分离**: 支持核心数据库按用途分离 (RAW, ROUTE, ADMIN), 支持异构数据库.
...
- **灵活日志配置**: 支持日志文件的大小, 路径等参数可配置;支持 Log 日志定期归档, 支持设置日志最大保留期限.

#### 11.7 图表展示工具

支持丰富的交互式图表分析能力.

- **图表交互**: 浏览数据时可放大和缩小, 并显示数据来源;当鼠标悬停在图表上时可以显示 Context 的数据;支持在数据或图表上标记注释;支持查看指定数据点及指定多个数据点的信息 (包含 Context, 数据值等).
...
- **辅助线与刻度**: 支持添加显示辅助线, 如 Alarm 和 Event 的辅助线;支持调整 Y 轴的刻度;Raw Trace Chart 与 SPC Chart 支持显示 SPEC 与 Step Line.
...
### 12. 数据收集 (Data Collection)

#### 12.1 最大激活采集计划数量设定

支持采集计划激活数量的上限管理.

- **最大激活采集计划数量设定**: 支持设定设备最多可激活的采集计划 (DCP) 数量.

#### 12.2 数据采集计划复制与移转

支持采集配置在设备间及环境间的快速复制与移转.

- **数据采集计划复制与移转**: 支持数据采集 (DCP) 配置过程中，当有新的参数发生重复时进行提示.
- **数据采集计划复制与移转**: 支持在不同环境之间导入导出数据采集 (DCP) 配置，如：将测试环境的设备采集计划转移到生产环境.
- **数据采集计划复制与移转**: 支持不同设备或子设备之间快速批量拷贝时，进行快速查找及批量替换指定字符信息.

### 13. 监控与建模 (Monitoring & Modeling)

#### 13.1 单变量 FDC 监控模型

支持更多单变量监控场景.

- **单变量 FDC 监控模型**: 支持对 IDLE 状态下的设备进行监控.

#### 13.2 汇总数据 SPC 监控模型

支持 SPC 图表的默认配置与分布检验.

- **汇总数据 SPC 监控模型**: 配置默认查看的图表，预设为原始数据图表，X-Bar/STDEV/MA/EWMA/Range...等基本 10 种图表.
- **汇总数据 SPC 监控模型**: 支持显示 Chart 数据 Mean/Median/Sigma 值，支持多种方式判断 Chart 数据是否符合正态分布，如：A-D/S-W/K-S Test.

#### 13.3 模型通用功能

支持模型功能开关的细粒度控制.

- **模型通用功能**: 模型建立后默认生成 Chart，可以为单个模型设置不同的功能开关，分为 Activate(是否判断 SPEC)、Interlock(是否执行 OCAP).

### 14. 数据分析与报表 (Data Analysis & Reporting)

#### 14.1 实时数据收集状态

支持采集计划激活状态的总览与下钻.

- **实时数据收集状态**: 支持查看 DCP(数据收集计划) 激活总数，并下钻至特定设备 / 腔室查看参数收值的实时状态.

#### 14.2 追踪数据分析

支持生产历史数据的标记, 着色与灵活绘图分析.

- **追踪数据分析**: 支持在报表上根据模型监控结果给每个 Run 的历史记录自动着色 (如：红色代表触发 SPEC)，并置顶显示发生 Fault Alarm 的 Run.
...
- **长期追踪数据分析**: 支持绘制至少三个月的追踪数据并进行趋势分析 (Long Term Trace)，最多可支持展示半年 Raw Trace 的 Trend Chart.

#### 14.4 建模覆盖率

支持 SPC 建模覆盖率的多维度查询.

- **建模覆盖率**: 支持按照区域、设备组、设备、子设备、时间范围等条件，查询 SPC 建模覆盖率 (包含参数个数、主模型与子模型数量、激活数量、Interlock 启用数量).

#### 14.5 系统使用率

支持系统功能使用情况的统计报表.

- **系统使用率**: 支持按照区域、设备组统计用户各功能菜单及按钮使用的次数 / 频率，并提供操作统计报表，便于推广系统.

#### 14.6 图表操作

支持更丰富的图表交互操作.

- **图表操作**: 支持以不同条件组合进行拆分图或拆分色，条件包含参数 (Parameter)、生产配方 (Recipe)、Lot, Substrate…等.
- **图表操作**: 支持将所有的数据进行 X 轴方向对齐.
- **图表操作**: 支持将图表和数据点信息复制到剪贴簿.

### 15. 报警管理 (Alarm Management)

#### 15.1 自定义报警模版

支持报警通知模板的自定义.

- **自定义报警模版**: 支持报警通知模板的自定义，可以根据需求设定对应的通知字段信息和通知发送排版方式.

#### 15.2 一键启停报警

支持告警功能的一键启停.

- **一键启停报警**: 支持告警功能一键 Enable/Disable.

### 16. 系统通用功能 (System General)
...
- **报警根因智能归因**: <mark> 同一时段产生的多条报警，由 AI 结合各自关联的 Tool, Chamber, Recipe, Step 与 Context 做聚类，识别成因相同或出自同一工艺步骤的报警簇，输出根因排序与建议核查方向；归因结果与报警记录一并保存，用于后续复盘及规则优化.</mark>
- **模型健康度智能体检**: <mark> 对已激活的 UVA/MVA 模型，AI 按期复核规格宽度、样本分布漂移与激活状态，标记长期未报警或频繁误报的模型并给出复核建议. 该检查面向模型投入使用后的持续有效性，与建模阶段的算法选择相互独立.</mark>
- **告警规则智能推荐**: <mark> 以历史 OOC/OOS 记录、人工处置结论与参数分布变化为依据，AI 就 Alarm Rule 的新增、合并或阈值调整提出建议，并说明每条建议所依据的数据范围与样本数量；建议经仿真测试和人工确认后方可启用，不自动写入规则库.</mark>
- **自动限值辅助决策**: <mark> 批量自动生成 SPEC 时，AI 参考该腔室历史数据的分布形态、工艺变更记录与既有规格的命中情况，就 Sigma 倍数、百分比或 Delta 的取值给出候选区间，并提示样本量不足、分布异常等需要另行判断的情形，供 PE 确定训练样本组与限值方案.</mark>
- **自然语言取数与趋势判读**: <mark> 支持用户以自然语言描述分析意图，由 AI 调用 FDC 既有功能进行检索与绘图，并对数据趋势给出判读，涉及长期趋势的判读同时列明所依据的数据点与筛选条件，便于复核. 支持记忆并生成定时任务功能.</mark>
- **vFAB 北向接口预留**: <mark> 支持事件、状态查询、受控动作、回写模型等接口预留 (包括但不限于 OOC/OOS 报警、模型激活 / 失活、自动限值生成完成、报警簇与归因结果、模型健康度清单、指定参数 Trace/Summary、告警规则启用确认回调、归因采纳结果回写等).</mark>
...
@see: ## FDC Function List → ### 6. 模型管理 (Model Management) / 故障侦测 (Fault Detection) → #### 6.5 模型配置与实时侦测
...
- **管控列表与 ByPass**:
    - 支持设定 Recipe/Sequence 管控列表 (Under Control 列表), 默认纳入管控;支持设定 ByPass Recipe, 此类 Recipe 不进行比对;支持设置 SubRecipe 是否进行卡控; <mark>未纳入管控的程序可按 Lot Type 施加卡控</mark>; <mark>支持配置 Recipe 运行完毕后是否自动从机台删除</mark>; 支持 Bypass, Cancel Bypass, Delete 所选中程序.
...
- **外部系统整合**: <mark> 支持与外部签核系统整合; 支持整合工厂现有签核系统.</mark>
...
- **签核可视性**: <mark> 支持设定关键参数, 签核人员可以查看参数内容.</mark>
...
- **Recipe 差异智能解读**: <mark> 系统得出的 FullBody, CheckSum 或 Parameter 比对结果，由 AI 转写为可读的差异说明，区分关键参数与一般参数，指出变化方向、幅度，并提示可能影响的工艺环节；说明与比对记录关联保存，供用户与签核人参考.</mark>
- **变更影响面分析**: <mark>Recipe 或设备常量（EC）版本发生变更时，AI 依据设备、机型、产品与 Recipe 之间的关联关系及历史履历，梳理受影响的设备范围、可能引用的产品与在制批次，形成影响清单提请复核，减少凭经验判断造成的遗漏.</mark>
- **程序解析辅助**: <mark> 对 Parse Adaptor 尚未覆盖或结构不规则的设备程序，AI 辅助识别参数段落与数据类型，就适用的比较方式（Absolute, ByRange, ByTolerance, ByList）及需要关注的关键参数给出建议，减轻新机型导入时的人工整理工作；解析结论经人工确认后方纳入模板.</mark>
- **签核要点提示**: <mark> 待签核的 Recipe 提交后，AI 依据历史签核记录、既有管控规则与本次变更内容，列出需要重点确认的条目以及过往同类变更曾被驳回的原因，帮助签核人快速定位审查重点；签核结论仍由签核人独立作出.</mark>
- **履历自然语言查询**: <mark> 支持用户以自然语言检索 Recipe, Sequence 与设备常量的变更履历和校验记录，例如查询某机型近三个月内参数规范的全部修改及其发起人，由 AI 转换为查询条件并整理返回结果，检索范围限于内网数据.</mark>
...
支持账号接口对接、操作审计及开放的系统接口, 实现安全可控的远程操作.

- **账号系统模块**: RCM 系统须能够对接本司账号系统, 并拉取部门结构, 由管理员设定不同部门或人员权限.
...
- **业务接口整合**: 可自定义开发接口, 与 EAP, MES 等上层系统作系统信息交互.
...
支持图形化的流程开发, 调试与业务接口整合能力.

- **图形化开发**: 支持图形化 workflow, 提供图形化的接口与其他业务接口整合, 开发的流程间可嵌套, 提高重用性.
...
Multi-Lot : 一个载具可容纳一个或多个批次, 一个批次只能在一个载具内.(试验线载具不多，要求能够混批放)
...
- **区域与设备对应**:
    - 支持 FAB/ 区域 / 设备对应关系定义, 设备可分配到不同区域, 用于工艺工程师 / 设备工程师 / 制造等不同目的; 支持 Fab, Area, Bay, Bank 等定义, 及 DIFF, Litho 等工作区域定义.
    - Work Area 支持用户根据需要自定义, 不限于固定的 Physical Location.
    - <mark>FAB 下的 department 和 section 需特殊定义 (如 Etch PE: FABA 定义为 F1ETCH-ETCHPE, PE Area 定义为 F1ETCH-ETCHPE, EE Area 定义为 F1ETCH-ETCHEE).</mark>
- **多工厂建模**:
    - 支持多工厂建模及生产 (8 英寸及先进封装工厂、微组装工厂), 各工厂独立且批次产品、流程、批次隔离, <mark>支持后续新工厂模型扩展建立和生产流片运转; 支持产品在多工厂切换加工</mark>.
    - 支持通过 FABSite 定义每个 FAB 的名称, 针对每个 FAB 设置不同的权限组, 通过权限组给用户赋权, 实现多 FAB 权限切换.
...
- **产品组与 Reticle 设置**:
    - 支持定义产品组 (产品的 Type 定义), 不同工艺的产品类别, 以及 Reticle 的设置 (能定义不同产品的 Photo Layer 的光罩设置); 支持产品状态定义及转换.
    - <mark>Flow 上 product 支持配置多个 Reticle Group, 功能相似的某一 Group 不能使用时自动切换到另一个 Reticle Group.</mark>
- **编码规则**: 支持自定义产品编码规则.
...
- **设备 Unit 定义**:
    - 支持 Chamber, <mark>Header, Tank 等设备 Unit 的定义</mark>.
...
- **版本与历史**:
    - Process, Product, Subplan 要具备版本控制功能, 都要具备历史查询详细的记录; Flow 支持版本升级, 默认使用最新版本; 需要具备人性化的 Flow 版本比对功能; <mark>支持比较两条工艺流程 (不同流程或不同版本) 并标记不同之处.</mark>
...
- **超限授权**: <mark>Rework 次数到了, 支持超级权限继续做 Rework.</mark>
...
支持 EDC Plan 各对象的建模、复用、版本控制与关联使用, 以及与 SPC 的双向接口对接.
...
- **SPC 管制计划协同**: 支持将 EDC Plan 内容送 SPC 定义管制计划收集数据的过滤条件 (按设备、子设备、配方、产品、流程), 接受 SPC OOC 触发的 OCAP 流程.
...
- **时间与动作设定**:
    - 支持设定最小时间、预警时间、最大时间, 各时间点可设定暂停、发邮件、加急等一个或多个并存动作, 邮件支持不同邮件组、收件人; 支持多步之间的 Q-Time, <mark>当系统超过 Q-Time 时自动触发, 不同的时间触发不同的警告.</mark>
...
- **设备属性定义**: 支持定义主、子机台属性, 含污染等级、状态切换模型、加工类型、最大加工能力、单次并行加工能力 (Batch) 等, 及机台可接受的载具类型; 设备最大加工批数、片数均可管控. 支持在 Chamber 上按污染等级进行多腔管理以及多污染等级管理.
...
- **数据清除与存档**:
    - <mark>提供终端用户及 IT 进行设备主数据及历史数据删除与存档功能</mark>, 不影响报表数据库及现有批次流片与查询.
...
- **分配载具 (Assign Carrier)**:
    - 允许用户将 <mark>Carrier</mark> 分配给没有 Carrier 的批次, 指定的载体必须是 Free 的, <mark>晶片的顺序可以在该功能中更改.</mark>
...
- **槽位信息变更 (Change Slot Map Info)**: <mark> 允许用户更改同一 Carrier 中晶片和原始批次之间的关系.</mark>
...
支持光罩全生命周期、状态控管、检测周期及与 EAP 接口对接.
...
- **Reticle Pod**: 工程师可以为 Reticle 添加一个 Pod ID, Pod 信息包括 Pod ID, Pod 类型, Pod 容量, 清洁周期等; 支持与 EAP 接口对接的光罩盒位置管理, 支持手动移动; 光罩盒 (Reticle Pod) 清洗时间 / 清除时间管理.
...
- **多订单类型**: <mark> 支持外延订单、晶圆订单、划片订单、晶圆复测订单、芯片复测订单等各种订单的批次创建.</mark>
...
- **邮件通知**: <mark> 扣留时发邮件通知相应角色里所有人员, 默认自动发送对应角色人员, 同时支持额外手动勾选具体人员.</mark>
...
- **两段式入库**: <mark> 入库时先由制造部入库, 再由仓库人员操作入库确认, 入库进度可查询, 入库信息可通过接口抛转 ERP.</mark>
- **退库**: 支持入库后退库返回现场的操作.
...
- **By ProductChange Reassign**: <mark> 按 Product 查询和选择一个或多个 Lot, 指定目标产品 (使用产品当前激活的流程和版本) 进行 Reassign, 系统自动 Reassign 每个 Lot 到目标产品和流程上, 并定位到与 Lot 当前 Step Seq 相同的 Step; 如果目标产品流程上没有 Lot 对应的 Step, Reassign 失败并提示, 可选择是否保留 FutureHold.</mark>
...
- **NPW/PW 管理**:
    - <mark>Change Product</mark> 时 NPW 与 <mark>PW</mark> 需分为两个功能模块, 人员权限等级不同; NPW 不需要走签核.
...
- **SPC 协同**: 支持 RC EDC 数据采集进 SPC, 并且支持手动勾选开关是否进 SPC.
...
- **触发方式**: 生产工步加工中批次或设备发生异常、批次 Running Hold 时可触发 RRC; 支持手动触发, 开立流程可开给工艺、设备进行签核. 触发 Season 的条件需包含 Recipe Change.
...
- **EAP 协同**: 支持使用 EAP 操作并自动收值.
- **数据查询**: <mark> 支持查询从 SPC 查询导入的 OFFLINE Chart 数据.</mark>
- **污染管控**: 支持污染等级管控.
...
- **Barcode 管理**:
    - 支持 Operator 通过扫描枪扫描 VendorBarcode; <mark>支持 Vendor Barcode 解析为 FAB 内部 Barcode (规则需客制化, 先默认手动录入)</mark>; 支持将 Barcode 存入 PRMS 系统中, 连同打印机将 FAB 内部 Barcode 作为瓶身的唯一识别码; 扫描光阻瓶身条形码, 在系统生成光阻瓶编码 (该瓶光阻唯一标识).
- **有效期管理**: 支持光阻有效期管理.
...
- **报空处理**: 支持机台报警时自动切换为 Empty (业务接口整合), 支持机台报警时手动将光阻报空.
- **新光阻录入**: 支持录入新的光阻; 支持推荐新光阻 BarcodeList (旧光阻同样 ResistNo 的 Status=Ready 的光阻瓶, 并且按照 ReceiveTime 从小到大排序).
- **更换联动 (PR Change)**: 光阻更换后, 旧的光阻瓶 Status 变为 InUse, 并记录 EQPID 和光阻管路; 光阻过期, 需更改 MES 机台状态 (业务接口整合); 更换时校验新老光阻是否符合要求, 不符合则退回; 同型号按领入时间先进先出优先使用最先领入的光刻胶; 更换完成后自动更改两瓶光阻状态; 可更换单个管路光阻.
- **过期管控**: 光阻超过有效期后自动扣留.
- **履历**: 光阻状态改变及用户操作均产生履历.
...
- **SPC 接口触发**: 与外部 SPC 系统接口对接, OOC/OOS 时自动触发扣留批次、对应工艺机台并自动生成 OCAP 单据及相应 OCAP 流程; 接口对接 SPC, 显示对应管制图.
- **OCAP ID**: 支持针对批次自动生成相关 OCAP ID.
- **签核协同**: <mark> 产生 OCAP 后与质量异常单签核业务接口整合, 将批次信息 (批次、段、站点、OCAP 原因等) 传递至外部签核系统, 指定步骤 (如判断重测是否 OOC/OOS) 支持传递至签核系统.</mark>
...
- **OCAP操作**: OCAP 需支持锁定, 同一时间仅第一个操作的人做后续处理.
...
- **协同业务**: 支持领料, 退料, 工单下达 (将已经下达的生产订单传输给 MES 系统), WIP 报工, 报废, 入库, 退库, 工单完成, Product ID Change, Lot Type Change 及其他与 ERP 系统接口对接的功能.

#### 16.2 跟 APC 协同

支持与 APC 系统的接口对接.

- **APC 协同**: 与 APC 系统接口对接, 实现相关功能协同. 接收 EDC 与量测结果, 输出配方修正建议至 EAP/ 设备, 形成闭环控制.

#### 16.3 跟 FDC 协同

支持与 FDC 系统的接口对接.

- **FDC 协同**: 与 FDC 系统接口对接, 实现相关功能协同.

#### 16.4 跟 YMS 协同

支持与 YMS 系统的接口对接.

- **YMS 协同**: 与 YMS 系统接口对接, 实现相关功能协同.

#### 16.5 跟 DMS 协同

支持与 DMS 系统的接口对接.

- **DMS 协同**: 与 DMS 系统接口对接, 实现相关功能协同.
...
- **拆批 / 合批**: <mark> 支持 HOLD 状态下拆批, 支持作业中拆批; 支持 HOLD 状态下合批, 支持作业中合批.</mark>
...
- **返工计数**:
    - FT 步操作中显示返工次数 (子批从父批复制), <mark>超过返工计数显示警告</mark>并保持.
...
- **良不良分流**: <mark> 不良品随良品整卷一起流通入库, 良品进良品仓, 不良品进废品仓.</mark>
...
- **良率统计**: 支持切割良率与崩边率统计 (录入以及与 EAP 接口对接).
- **Recipe 管控**: 支持切割 Recipe 管控 (与 EAP 接口对接).
...
- **统计与存放**: 支持挑选率 (拾取率) 统计 (录入以及与 EAP 接口对接); 支持挑选芯片存放方式, 包含 Reel 卷带, Tray 盘等.
...
- **防错管理**: 支持贴装参数防错, MES 定义产品 / 设备 Recipe, 与 EAP 接口对接, 系统自动校验和下发 Recipe; 支持治具防错贴装吸嘴 / 顶针管理, 记录贴装头吸嘴, 顶针等治具的使用次数, 到期自动预警或锁定.
...
- **Recipe 与 Map**: 支持塑封 Recipe 管控 (与 EAP 接口对接); 支持 Strip (Wafer) Map 管理.

#### 18.5 激光打标站点管控

支持激光打标工序的作业管控.

- **打标管理**: 支持激光打标数据源配置 (参考标签数据源配置); 支持打标数据防错防呆 (与 EAP 接口对接); 支持 Strip (Wafer) Map 管理.
...
支持物料与 BOM 的完整管控体系、多维度管理、消耗追溯及与 ERP 接口对接.

- **物料分类**: 支持物料分类维护 (主要材料、间接材料、辅料等) 及物料编码维护, 工步配置对应材料.
- **BOM 体系**: 针对物料和产品可定义物料的管控属性 (例如化学品的时效等); 可承接 ERP 的产品 BOM 形成 MES 的产品 BOM, 并基于产品 BOM 结合工艺流程形成生产 BOM (工单 BOM, 工序 BOM); 在 MES 系统可自行创建 BOM (工单 BOM, 工序 BOM); 产品, 物料, BOM 可启用版本管理; 支持 BOM 分类管理 (返工、试制、量产、替代、设备维修等)、版本、状态、适用范围管理, 与 ERP BOM 功能有效配合、接口对接, 可在产品族、产品、制造资源、批次等对象定义 BOM 结构, 支持变更追溯及变更履历记录.
- **工序物料**:
    - 支持设置工序消耗的物料 BOM; <mark>支持设置替代料和替代顺序</mark>; 可按工步设置、查询, 支持版本及激活权限控制.
...
- **ERP 协同**: 与 ERP 仓库管理模块接口对接, 共同完成线边仓、发料、检查、退库、物料 Q-Time 等业务.
...
- **出货条件检查**: 可与 WAT, Defect, SPC, Lot Type 等其他业务接口整合检查批次是否符合出货条件, 出货条件可维护在 MES 或其他系统.
...
- **EAP 联动**: <mark> 部分功能需要联动 EAP 实现.</mark>
...
- **ECM 单号与升版管控**: 引入 ECM (Engineering Change Management) 单号管理机制, 涵盖创建 (Create)、验证 (Validation)、冻结 (Frozen)、批准 (Approve)、发布 (Release) 五个阶段, 确保 Flow 升版过程受控; 版本管控覆盖类似 Product, Top Plan, Route 层级, 支持对线上 Lot 的热站升版.
...
- **APS 排产结果协同**: MCP 与 APS 排产业务接口整合: MCP 本身不执行排产运算, 但需承接并录入 APS 排产结果 (各段制造工单的下线时间点、完工时间点及段间 Buffer), 据此驱动各工艺段制造工单执行与进度跟踪.
...
- **企业级与 Fab 级 MAP 分发架构**: 独立的 MAP 模块, 分企业级与 Fab 级: 企业级 MAP 接收外部 (客户 / 供应商) Map 数据并做标准化, 再分发至各 Fab 级 MAP, 与各厂 MES / EAP 串联使用.
...
- **Wafer 与 Die 双单位管理**: <mark> 同一 MES 平台底层同时支持 Wafer 级 (前道) 与 Die 级 (后道) 双重加工与追溯单位, 使前中后道混合的多条产线可在一套系统内建模与切换查看, 无需前道 / 后道分别部署两套系统.</mark>

### 39. EAP / 设备自动化接口 (SECS-GEM)

支持 MES 与设备自动化 (EAP) 之间的 SECS/GEM 标准通信与设备接口对接管理.
...
- **PM 计划与调度**: 支持预防性维护 (PM) 计划定义, 按运行时间 / 次数 / 日历周期触发, 自动生成 PM 工单并管控设备 PM 状态与加工限制. PM 执行需具备严格防呆机制, 确保严格按照计划进行. PM 到期预警的触发方式支持自定义, 如 By 片数触发、By RF Time 触发、By 关键 Parts 使用寿命触发等; 系统统一高亮显示已触发的保养计划, 支持提前预警并同步发送通知.
...
MES 与 AGVC (AGV 搬送控制) 系统的模块需求.

- **AGV对MES的需求**: <mark> 提供与 AGVC 的通信能力, MES 为 Active (主动端).</mark>
- **任务下达**: <mark> 可向 AGVC 下达任务, 包含任务 Id, 物料号, 起点、终点、优先级.</mark>
- **储位分配**: <mark> 支持接收电子料架状态并对任务分配具体的储位.</mark>
- **料架储位状态显示**: <mark> 支持 MES 界面显示料架的储位的状态.</mark>
- **异常任务处理**: <mark> 具备异常任务处理机制 (MES 接收到 AGVC 上报的任务进度及状态后做出相应的处理).</mark>
- **任务反馈与履历**: <mark> 支持接收 AGVC 任务反馈, 并支持查询以及保存历史记录.</mark>
- **任务控制指令**: <mark> 可下达任务控制指令 (如 Cancel, Abort), 确保 AGVC 可以收到且正确分析指令.</mark>
- **任务目的地变更**: <mark> 在 AMR 自动搬送小车需要更改任务目的地的场景下, 支持先下 Abort 指令, 再重新创建任务.</mark>
- **料架载具信息接收**: <mark> 支持接收电子料架上报的载具信息 (如载具清单、ID、编号、类型、当前位置编号、当前区域编号).</mark>
- **批量任务下达**: <mark> 支持针对 Batch 设备的货物运输下达多个任务指令.</mark>

### 58. MCS 模块

MES 与 MCS (天车搬送控制) 系统的模块需求.

- **任务重发幂等**: <mark> 支持因网络超时、界面重试或人工操作等异常情况再次发送同一笔搬运任务需求, 此时需沿用原任务标识或明确提供原任务关联标识, 不得生成一笔看似全新的搬运.</mark>
- **任务冲突处理**: <mark> 支持 MES 发现针对目标下达的新任务与旧任务的冲突; 当两个任务部分信息 (如 Carrier、来源、目的地等) 不一致时, 不能直接改已预约 Job 里面的值, 需根据流程 Reroute、或 Cancel 后新建任务指令、或明确提示业务冲突.</mark>
- **任务状态区分**: <mark> 针对 MCS 返回的各类信息 (如“通信已收到”、“任务已受理”、“执行中”、“完成”、“失败”等), MES 需分别记录并展示其不同的业务含义, 不能据此判定任务已完成或 Carrier 已到达.</mark>
- **请求受理结果处理**: <mark>MES 需取得 MCS 对搬运请求的接受或拒绝结果并对回复进行处理; 如果 MCS 拒绝执行命令, MES 需按流程决定人工处理、调整后重送或停止派工.</mark>
- **搬送事件处理**: <mark>MES 可接收 MCS 上报的信息 (Carrier、任务标识及事件标识, 如“搬送信息”、“位置变化”、“到位”、“失败”、“取消”等), 并依据此类信息处理事件, 保证业务信息状态可维护追溯.</mark>
- **重复乱序事件识别**: <mark>MES 收到特殊情况 (断线恢复、系统重启或补发) 导致的重复、迟到或乱序事件时, 能识别已处理事件或旧版本状态, 不得因重复事件再次下达搬运, 也不得以旧状态覆盖新状态.</mark>
- **搬运任务取消**: <mark>MES 能取消尚未完成的搬运任务, 取消请求需指向原任务标识, 且 MES 需等待接收 MCS 的回复信息 (如“已接受取消”、“无法取消”、“已完成”、“已终止”等结果).</mark>
- **目的地变更**: <mark> 当 Carrier 目的地改变后, MES 需提供原任务标识、新目的地及变更版本或顺序信息, 将同一 Carrier 改送至新目的地, 且需等待天车 MCS 对更改目的地结果的明确回应.</mark>
- **任务优先级调整**: <mark> 当 MES 需调整未完成任务的优先级时, 优先级调整必须指向已有任务, 不得把旧任务 Cancel 后新建任务替代; 同时 MES 需接收变更已接受、未生效或不支持的结果.</mark>
- **多载具批量任务**: <mark>MES 可一次下达多个 Carrier 的搬运任务, 且必须选择该批任务是“整批预检均通过才执行”或“允许逐笔接受 / 拒绝”, 每个 Carrier 都需有独立的结果.</mark>
- **位置上报与对账**: <mark>MES 支持实时接收 MCS 上报的 Carrier 位置变化, 必要时可按 Carrier 查询位置或参与项目定义的库存 / 位置对账流程.</mark>
- **搬送属性管理**: <mark>MES 必须明确维护各类业务属性 (Carrier Type、批次属性或其他搬送判断所需信息) 的属性来源与更新时机, 并有明确的属性缺失处理规则, 属性缺失时不得下达可能错误的搬送任务指令.</mark>
- **跨MCS中转搬送**: <mark> 同一 Fab 存在两套 MCS、搬运起点与终点分属不同 MCS 时, 需经中转 STK 或交接站点; MES 或项目约定的协调组件需维护设备归属与交接站点规则, 跨区任务不得直接要求两套 MCS 同时执行同一段物料搬送; 中转前段完成、Carrier 到达交接位置并满足续派条件后才可触发后段.</mark>
- **跨区派工异常管控**: <mark> 支持在收到交接站点、站点所属设备或归属信息异常时, MES 停止受影响的跨区派工并保留待处理任务.</mark>
- **断线重连与对账**: <mark> 面对特殊情况 (如 MES 与 MCS 连接中断、网络抖动、MES 重启或 MCS 服务切换), MES 需自动重连至项目约定的 MCS 逻辑服务地址; 恢复后使用原任务标识查询结果、接收补发事件或进行对账.</mark>
- **未知结果处理**: <mark>MES 必须有对未知情况 (如已发送请求但未收到业务回复、已收到事件但无法确认是否完成处理等) 的处理机制: 优先使用原任务标识查询或等待补发, 不得直接以新任务标识重复下单.</mark>
- **自研中间件规格**: <mark>MES 使用自研或非通用通讯中间件与 MCS 连接时, MES 厂商需提供中间件技术规格、连接与订阅方式、报文样例、错误码、超时重试、断线重连、版本规则, 以及可供联调的环境、模拟工具或技术支持窗口; MCS 可配合适配; 中间件能力说明、缺陷定位及中间件侧修正由 MES 厂商负责.</mark>
- **通信与ACK语义**: <mark>MES 与 MCS 采用请求 / 回复、事件上报或 ACK 确认方式通信; MES 厂商需明确定义 ACK 仅代表“通信收到”、还是“进入处理”或“业务接受”, ACK 与业务完成不得混用.</mark>
- **逻辑地址接入**: <mark> 在网络、服务器或接口实例切换中, MES 不得固定连接单一 MCS 物理服务器, 需使用双方 (MES & MCS) 约定的逻辑地址、虚拟地址、服务名称或等效接入方式.</mark>
- **版本发布协同**: <mark>MES 厂商发布版本更新 (如接口字段、枚举、通讯中间件等) 前, 需向天车厂商提供版本号、差异说明、受影响接口、样例报文、兼容策略、回退方式及回归测试计划.</mark>
- **联调测试支持**: <mark> 项目进入接口联调、FAT, SAT 或生产切换时, MES 厂商需提供接口技术窗口、问题响应机制、测试账号 / 环境、测试数据准备及上线支持安排.</mark>
- **容量性能承诺**: <mark> 在特定生产情景下 (如正常生产、满载生产、短时突发或断线后事件补发等), MES 厂商需提供其可承受的请求量、事件量、同时未完成任务数、事件积压与恢复补发能力, 并参与项目容量测试.</mark>
- **异常日志与告警**: <mark> 发生异常状况时 (如接口错误、积压、重试过多、连接异常或业务拒绝率异常等), MES 需提供必要日志、追踪标识、连接状态、积压量、错误码及告警信息, 供 MES 和天车联合定位; 敏感信息需按项目要求脱敏.</mark>

### 59. 其他业务协同

- **与泛微门户协同**: <mark> 需包含门户管理、协同管理等模块协同.</mark>
- **与用友 ERP 协同**: <mark> 需包含财务管理 (工单 / 物料 / 成本) 等模块协同.</mark>
- **与 WMS 协同**: <mark> 需包含物料出入库、库存管理等模块协同.</mark>
- **与 PLM 协同**: <mark> 需包含产品数据、工艺数据、BOM 管理等模块协同.</mark>
- **与帆软报表协同**: <mark> 需包含报表管理等模块协同.</mark>
- **与文档档案系统协同**: <mark> 需包含文档管理、档案管理等模块协同.</mark>
- **与电子签章平台协同**: <mark> 需包含电子签章 (CA) 等模块协同.</mark>

### 60. 智能化需求

- **AI 感知与预测**: <mark> 耗材与治具寿命预测: 以 Reticle PD、探针卡、治具与光阻锡膏的使用次数、计时与良率衰减预测剩余寿命，提前触发更换与补货。数据：PD 使用次数与 IRIS、扎针次数、挂载计时、光阻有效期与用尽记录.</mark>
- **AI 感知与预测**: <mark> FutureHold 计划扣留智能化: 由前道 EDC 量测、站点良率与设备履历预测批次失效风险，自动在取样站点预置扣留条件，替代靠经验手工设 FutureHold。数据：FutureHold 设定与生效时机、产品级规则、EDC 量测、Hold 履历.</mark>
- **AI 感知与预测**: <mark> 工单完工与交期预测: 由在制 WIP、设备可用率与历史周期时间预测完工日期与超期风险，为计划与销售提供可信交期.数据：工单管理、WIP 与周期时间管理、设备状态履历.</mark>
- **AI 治理与自评估**: <mark> OCAP 处置效果自评估与推荐: 按历史同类异常的处置动作与最终效果推荐 OCAP 范围与动作，避免一律整批 Hold.数据：OCAP FLOW 设置与执行履历、处置结论、后续良率结果.</mark>
...
- **数据过滤**: 支持 OOS, OOC 等多种过滤功能, 被过滤 (排除) 的数据默认不参与 Cpk 统计.
...
- **签核协同**: 支持 Limit 的手动接收或拒绝, 也支持与其他签核业务接口整合.
...
- 数据流是单向的，Chart 是采集结果的记录，修改采集后的内容不影响后续数据收集
...
- chart 的点位数据信息量大，自定义过滤条件可以进行快速筛选
...
- 设备 Down 或 PM 后，进行 monitor 的量测值需要进行特殊标记，可对其进行特殊的 SPEC 管控
...
支持计划性与非计划性、多触发类型的维修保养管理及外部业务接口整合.
...
- **预测性维护**: <mark> 基于历史故障数据与实时传感器时序数据，采用学习算法预估 Part 剩余使用寿命（RUL），预判未来故障时间点，而非仅依赖当前状态阈值告警，实现从被动维修向预测性维护转型.</mark>
...
- **vFAB 北向接口预留**: <mark> 支持事件、状态查询、受控动作、回写模型等接口预留 (包括但不限于保养触发、预警推送、保养工单状态变更、Part RUL 预测值与健康度评分、调整保养工单等).</mark>
...
支持与 MES, EAP, SPC, ALARM, MCS, EMS 等多业务接口整合.

- **MCS 协同**: 支持搬运任务下发, 实现自动化搬运.
- **MES 协同**:
    - WIP 清单: 可加工工单信息清单.
...
- **周边业务接口整合**: 支持与 MES, APC, PMS 等其它周边系统 (ERP, MCS, Signoff, YMS) 之间的直接交互与实时信息交互.
...
- **外部数据导入**: 支持从外部系统导入的接口对接工具, 例如 Flat 文件、CSV 格式文件或者其他数据库.
...
- **Excel 宏协同**: 支持与 Excel 宏 (Macro) 接口对接.
- **R 语言协同**: 支持与 R 语言接口对接.
- **Python 协同**: 支持与 Python 接口对接.
...
实施费用: 参照 SOW / 实施费用与产品授权的比例预估约为 1:1

EAP

产品授权: 按工厂机台数阶梯式授权

实施费用: 按 template/eqp_type/roll_out 数量计价；芯源设备要支持 2"~12" 不同产品的生产，EAP Type 数量较多；芯源相当数量的设备是非标，较符合 SEMI 标准的设备实施费用高

FDC

产品授权: 按需求机台数阶梯式授权

实施费用: 按 eqp_type/roll_out 数量计价

RMS

产品授权: 按工厂授权

实施费用: 按 eqp_type 数量计价

APC

产品授权: 按 Litho, CMP, Etch, TF, Fur, IMP 区授权；按客户需求的区域来报价
...
产品授权: 按机台数梯度报价（500 台以内一个报价）

实施费用: 按区域报价（如果多个区域派工规则和流程一样算一个区域），NPW 管理等其他派工流程算一个区域
...
CMMI 证书: 分 3/5 级, 5 级加分 (类似的 人员证书，诸如 PMP，企业资质，诸如 ISO27001 之类, 政采不行, 企业采购才行得通)

业绩证明: 3 个 12 吋 工厂实施, ≥3 加分, 不满足扣分
