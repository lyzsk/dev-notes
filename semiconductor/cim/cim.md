...
- **vFAB 北向接口预留**: <mark> 包括但不限于设备/腔体状态切换、通讯断连与恢复、异常干预触发等事件发布, 设备实时状态、载具与 Load Port 状态、当前跑货 Job 与 Recipe（PPID）等状态查询, 跑货方案确认/驳回、异常干预指令受控下发等动作接口预留.</mark>
...
- **vFAB 北向接口预留**: <mark> 包括但不限于 OOC/OOS 报警、模型激活 / 失活、自动限值生成完成等事件发布, 报警簇与归因结果、模型健康度清单、指定参数 Trace/Summary 等状态查询, 告警规则启用确认回调等接口预留.</mark>
...
- **vFAB 北向接口预留**: <mark> 包括但不限于 Recipe/EC 版本变更、签核通过 / 驳回、CheckSum 比对异常等事件发布, Golden Recipe 当前版本与签核状态、设备与 Recipe 对应关系、变更影响清单等状态查询, 签核流程提交 / 确认回调等接口预留.</mark>
