...
# 风险与待确认事项：MES 多工艺段系统整合

## 一、背景风险

本项目要求一套 MES 平台同时覆盖 **材料、外延、晶圆制造、封装、测试** 五大工艺段. 行业内个别厂商的前道与后道 MES 相互独立，为满足本公司 5 条产线的统一管控现状，需对相关系统进行整合.

系统整合涉及跨工艺段数据模型、流程主数据与接口的打通，**实施复杂度高**；整合完成后还需开展全链路联调与回归验证，**验证工作量大、周期不可控**，存在直接影响 9 月验收节点的交付风险.
...
如确需系统整合，功能培训必须安排在**整合完成、功能冻结之后**开展；否则培训所授内容与最终上线版本存在差异，容易被用户方 highlight 功能变更，培训效果与认可度将大打折扣.

### 2. 系统上线的硬性时间节点

须同步确认 **UAT（用户验收测试）环境**与 **PRD（生产）环境**的上线时间节点. 在整合周期未明确前，整体上线节点无法锁定，需厂商给出整合工作量与排期承诺.

### 3. 项目交付期资源投入与团队资质要求

鉴于 9 月验收节点倒排带来的交付压力，须明确要求厂商：

- 为本项目投入的总人力量、各层级人员（项目经理 / 技术总监 / 开发 TL / 测试 TL / 开发工程师 / 测试工程师等）的**级别分布与到岗人数**；
- 提供上述全部人员的**详细简历及同类项目经历**；
- 其中项目经理、技术总监、开发 TL、测试 TL 等关键岗位人员**须到现场面试**.

以此确保项目团队具备相应的整合实施经验与技术能力，能够支撑多工艺段 MES 系统整合及按期上线交付.

### 4. 验收后运维交接团队的配置要求

为降低 MIT 团队在项目验收后短期内的运维压力、保障系统平稳交接与持续运维，须要求厂商同步提供验收后驻场 / 支持运维团队的配置：

- 运维投入人员**总数**、各层级（运维主管 / 开发支持 / 现场运维等）的**级别分布**；
- 提供每名运维人员的**详细简历及同类系统运维经历**.

以此确保运维团队具备足够的能力和经验承接系统运维工作，避免验收后出现运维真空.
