...
| 材料             | 类型            | 优势             | 主要应用             |
| ---------------- | --------------- | ---------------- | -------------------- |
| Si(硅)           | 第一代半导体    | 成本低、工艺成熟 | CMOS、存储           |
| Ge(锗)           | 第一代半导体    | 高迁移率         | SiGe、探测器         |
| GaAs(砷化镓)     | III-V           | 高频             | PA, VCSEL            |
| InP(磷化铟)      | III-V           | 光通信           | DFB, EML             |
| SiC(碳化硅)      | 第三代半导体    | 高压、高温       | EV、储能             |
| GaN(氮化镓)      | 第三代半导体    | 高频、高效率     | RF、快充             |
| AlN(氮化铝)      | 超宽禁带 / 陶瓷 | 高导热           | 封装、DUV            |
| β-Ga₂O₃(氧化镓)  | 第四代候选      | 超高耐压         | 超高压器件 (研发)    |
| Diamond(金刚石)  | 超宽禁带        | 导热率最高       | 热管理、极端功率器件 |
| Sapphire(蓝宝石) | 单晶氧化物      | 成本低           | LED 衬底             |
| Si₃N₄(氮化硅)    | 陶瓷            | 高可靠性         | SiC 功率模块         |
...
PR Spec(Purchase Requisition Specification)(采购申请规格书 / 请购单规范)

PR Spec 是指企业内部部门向采购部提出购买需求时, 对所购物品或服务的技术参数、数量、期望交付日期等进行的详细描述

PO Spec(Purchase Order Specification)(采购订单规格 / 订单技术规范)

PO Spec 是指在正式发给供应商的采购订单中, 附带或引用的具体技术标准和验收规范
...
> 两类费用可以合并报价，也可以将定制化开发费用单独列为二类费用
...
- 持有 **软件产品销售许可证** 或 **软件著作权登记证书**
- 且 **同类产品可在市场上询价**

### 报价方式

- 可按照 **产品模块** 报价，功能层级划分至:**一级功能 → 二级功能 → 三级功能**
- **单一功能报价 ≤ 15 万元**
...
| ---------------------------- | ------------------- |
| 常规开发                     | 2 ~ 2.5 万元 / 人月 |
| 技术复杂（如 AI 智能应用等） | 最高 3 万元 / 人月  |

### 单条功能上限（最细颗粒度）

- 工作量:**不超过 5 ~ 6 人月**, 最小是 1, 最小变动单位 0.5
- 总金额金额:**≤ 15 万元**

## 二类费用

二类费用除了系统集成以外，通常是第三方服务费（咨询、监理、软测、安测、密测等）。系统集成费=（软件产品+ 硬件设备）\* 系数，系数一般取值 0.6，集成费里就涵盖了软件产品的实施部署、设备适配等费
...
```txt
┌─────────────────────────┐
│   SaaS（软件即服务）      │  ← 用户直接使用应用（如 Office 365）
├─────────────────────────┤
│   PaaS（平台即服务）      │  ← 用户开发应用（如云开发平台）
├─────────────────────────┤
│   IaaS（基础设施即服务）   │  ← 用户管理虚拟机/存储（如 ECS）
└─────────────────────────┘
```

```txt
┌──────────────────────────────────────────────┐
│  ⑥ MaaS 模型平台   —— 把大模型当服务直接调用     │  ← 消费智能
│  ⑤ AI 平台         —— 训练/管理/部署 AI 模型     │  ← 生产智能
│  ④ 大数据平台       —— 海量数据的存、算、分析     │  ← 处理大数据
│  ③ PaaS 平台        —— 应用的开发/运行/运维环境   │  ← 跑应用
│  ② 数据库           —— 结构化数据的存取与事务     │  ← 管数据
│  ① 操作系统         —— 管理硬件、提供运行底座     │  ← 管硬件
└──────────────────────────────────────────────┘
        ↑ 越往下越“基础/通用”，越往上越“专业/智能”
```

| 维度     |  操作系统   |   数据库   |    PaaS    |    大数据    |  AI 平台   |      MaaS       |
| -------- | :---------: | :--------: | :--------: | :----------: | :--------: | :-------------: |
| 管什么   |  硬件资源   | 结构化数据 |  应用运行  | 海量数据计算 |  模型训练  |    模型服务     |
| 主要用户 | 运维 / 系统 | 业务 /DBA  |  应用开发  |  数据工程师  | 算法工程师 | 业务 / 应用开发 |
| 技术门槛 |     高      |     中     |     中     |      高      |     高     |       低        |
| 兴起阶段 | 计算机诞生  | 信息化时代 | 云计算时代 |  大数据时代  |   AI 1.0   |   大模型时代    |

# 网络

| 网络名称                                             | 主要承载的系统 / 设备                     | 一句话作用                                  | 属性 / 安全级别 |
| ---------------------------------------------------- | ----------------------------------------- | ------------------------------------------- | --------------- |
| 内部工作网（办公网）                                 | OA、邮件、ERP/PLM 前端、HR、协同办公      | 员工日常办公                                | IT·常规         |
| 工控网（FMCS）(Facility Monitoring & Control System) | 厂务 PLC/DCS/SCADA, FMCS 服务器、现场仪表 | 监控厂房环境与厂务（水 / 电 / 气 / 洁净室） | OT 核心·最高    |
| 生产网                                               | MES, EAP, RMS, SPC/APC、机台联网、AGV     | 跑产品制造的数据流与指令流                  | OT 核心·最高    |
| 互联网                                               | 上网出口、对外服务、云对接                | 对外访问 / 被访问的边界                     | 边界·严控       |
| 安防网                                               | 视频 CCTV、门禁、周界、消防、停车、访客   | 物理安全（看人、看门、看周界）              | 专网·隔离       |
| 数据交换网                                           | 网闸 / 光闸、数据交换平台、文件摆渡       | 跨安全域受控交换数据（不直连）              | 边界·交换枢纽   |
...
| SCADA | Supervisory Control And Data Acquisition | 监控与数据采集系统（数据采集与监视控制系统） |

```txt
            ┌──────────────────────────────┐
   上层监视  │  SCADA  监控与数据采集          │  ← 看全厂、收数据、发调度
   调度层    │  （中控大屏 / HMI / 报警 / 历史）│     “眼睛 + 调度员”
            └──────────────┬───────────────┘
                           │ 采集数据 / 下发设定
            ┌──────────────┴───────────────┐
   过程控制  │  DCS  分布式控制               │  ← 连续量精细调节（PID 闭环）
   层        │  （温度/压力/流量/液位 稳态控制）│     “连续过程的总指挥”
            ├──────────────────────────────┤
   逻辑控制  │  PLC  可编程逻辑控制           │  ← 开关量/逻辑/联锁
   层        │  （启停、顺序、保护联锁）        │     “现场的开关大脑”
            └──────────────┬───────────────┘
                           │ 接传感器/执行器
            ┌──────────────┴───────────────┐
   现场层    │  仪表 / 阀门 / 电机 / 传感器     │  ← 真正干活的物理设备
            └──────────────────────────────┘
```
...
| 维度     | OT                                           | IT                                |
| -------- | -------------------------------------------- | --------------------------------- |
| 管什么   | 物理设备 / 过程（水电气、机台、产线）        | 数据 / 信息（文档、账目、流程）   |
| 关心什么 | 可用性、实时性、安全 (safety)                | 保密性、完整性                    |
| 出事后果 | 停产、设备损坏、人身 / 环境事故              | 数据泄露、业务中断                |
| 典型系统 | PLC / DCS / SCADA / FMCS / MES 控制侧        | OA / ERP / 邮件 / 数据库 / 办公网 |
| 更新节奏 | 多年不动，设备寿命 10–20 年，难打补丁        | 频繁升级、打补丁                  |
| 通信协议 | 工业协议（Modbus, PROFIBUS, OPC 等，多明文） | TCP/IP, HTTP 等通用协议           |
...
OHT(Overhead Hoist Transport)(overhead 天车 / 空中走行式无人搬运车)
...
```text
            ...
            └─────────────────────────────────────────┘
                              ...
                              │ 搬运指令
                       ┌──────┴──────┐
                       │  MES / OMS  │   ← 上层生产系统(告诉MCS"把这个lot搬到哪")
                       └─────────────┘
```
...
```text
   ┌──────────────┐   OHT天车    ┌─────────┐  OHT/AGV   ┌──────────┐         ┌────────┐
   │   Stocker    │ ◄══════════► │  轨道区  │ ═════════► │  E-Rack  │ ──机械手──►│ 机台LP │
   │ (中央大仓库)  │              └─────────┘            │(机台旁小架)│         │(LoadPort)│
   └──────────────┘                                      └──────────┘         └────────┘
        ↑ 长期存储、等排产                                    ↑ 短期缓冲、即取即用
```
...
`系统集成`、`系统运维`，`系统对接` 都是二类费用，所以最好用接近的词汇替换：`系统协同`、`接口对接`、等等
